...
public class CatalogDAOImpl 
    ...
    public  NamespaceInfoImplHb getDefaultNamespace() {
        Query query = buildQuery("from " , NamespaceInfoImplHb.class , " where default = " , param(Boolean.TRUE) );
    ...
    public  <T extends StoreInfo> T getStoreByName(WorkspaceInfo workspace, String name, Class<T> clazz) {
        Query query = buildQuery("from " , clazz , " where name = " , param(name), " and workspace = ", param(workspace));
        return (T) first(query);
    }
    ...
    public  <T extends ResourceInfo> T getResourceByName(String ns, String name, Class<T> clazz) {
        Query query = buildQuery("from " , clazz , " r where name = " , param(name) , " and r.namespace.prefix = ", param(ns));
        return (T) first(query);
    }
    ...
    public <T extends ResourceInfo> T getResourceByStore(StoreInfo store, String name, Class<T> clazz) {
        Query query = buildQuery("from " , clazz , " r where name = " , param(name) , " and r.store = ", param(store));
        return (T) first(query);
    }
    ...
    public  LayerInfo getLayerByName(String nsprefix, String name) {
        Query query = buildQuery("from ", LayerInfoImplHb.class, 
                " l where l.name = " , param(name),
    ...
    public  List<LayerInfo> getLayersByResourceId(String resid) {
        Query query = buildQuery("from ", LayerInfoImplHb.class , " where resource.id = ", param(resid));
    ...
    public  NamespaceInfoImplHb getNamespaceByPrefix(String prefix) {
        Query query = buildQuery("from " , NamespaceInfoImplHb.class , " where prefix = " , param(prefix));
        return (NamespaceInfoImplHb)first(query);
    }
    ...
    public  WorkspaceInfoImplHb getDefaultWorkspace() {
        Query query = buildQuery("from " , WorkspaceInfoImplHb.class , " where default = " , param(Boolean.TRUE));
    ...
    public  WorkspaceInfo getWorkspace(String id) {
        return (WorkspaceInfo) first(buildQuery("from " , WorkspaceInfo.class , " where id = ", param(id)));
    }
    ...
    public  WorkspaceInfo getWorkspaceByName(String name) {
        WorkspaceInfo ws = (WorkspaceInfo) first(buildQuery("from " , WorkspaceInfo.class , " where name = ", param(name)));
        return ws;
    }
    ...
    public void save(LayerInfo entity) {
        ...
        // FIXME we are replacing some referenced object here because hib would recognized original ones as unattached.
        if(entity.getResource().getId() != null) {
            Query query = buildQuery("from ", entity.getResource().getClass(),
                                    " where id = ", param(entity.getResource().getId()));
        ...
        if(entity.getDefaultStyle() != null) {
            Query query = buildQuery("from ", StyleInfo.class,
                                    " where id = ", param(entity.getDefaultStyle().getId()));
