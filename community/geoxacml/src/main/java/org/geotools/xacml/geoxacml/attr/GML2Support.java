...
public class GML2Support extends GMLSupport {
	...
	GeometryAttribute buildFromGML(Node gmlNode) throws Exception {
		...
		if ("LineString".equals(gmlNode.getLocalName())) {
			...
			GeometryAttribute attr = new GeometryAttribute(ls, srsName, gid,
					GMLVersion.Version2,GML_LINESTRING);
			return attr;
		}
		...
		if ("LinearRing".equals(gmlNode.getLocalName())) {
			...
			GeometryAttribute attr = new GeometryAttribute(lr, srsName, gid,
					GMLVersion.Version2,GML_LINEARRING);
			return attr;
		}
		...
		if ("Polygon".equals(gmlNode.getLocalName())) {
			...
			GeometryAttribute attr = new GeometryAttribute(poly, srsName, gid,
					GMLVersion.Version2,GML_POLYGON);
			return attr;
		}
		...
		if ("MultiPoint".equals(gmlNode.getLocalName())) {
			...
			GeometryAttribute attr = new GeometryAttribute(mp, srsName, gid,
					GMLVersion.Version2,GML_MULTIPOINT);
			return attr;			
		}
		
		if ("MultiLineString".equals(gmlNode.getLocalName())) {
			MultiLineString mls = parseMultiLineString(gmlNode);
			GeometryAttribute attr = new GeometryAttribute(mls, srsName, gid,
					GMLVersion.Version2,GML_MULTILINESTRING);
			return attr;			
		}
		...
		if ("MultiPolygon".equals(gmlNode.getLocalName())) {
			...
			GeometryAttribute attr = new GeometryAttribute(mp, srsName, gid,
					GMLVersion.Version2,GML_MULTIPOLYGON);
			return attr;			
		}
	...
	void encodeASGML(GeometryAttribute attr, StringBuffer buff) {
		...
		if (GML_MULTILINESTRING.equals(attr.getGmlType()))
			encodeMultiLineString((MultiLineString) attr.getGeometry(), buff,abstractGeometryAttributeMap);
		if (GML_MULTIPOLYGON.equals(attr.getGmlType()))
			encodeMultiPolygon((MultiPolygon) attr.getGeometry(), buff,abstractGeometryAttributeMap);
		
		
	}

	protected void encodeCoordinates(CoordinateSequence coordinates,
			StringBuffer buf) {
		...
		for (int i = 0; i < coordinates.size(); i++) {
			...
			boolean y = (coordinates.getDimension() > 1)
					&& !new Double(c.y).isNaN();
			if (y)
				buf.append("," + c.y);

			boolean z = y && (coordinates.getDimension() > 2)
					&& !new Double(c.z).isNaN();
			if (z)
				buf.append("," + c.z);
	...
	public Polygon parseBox(Node node) throws GMLException {
		...
		if (coords != null) {
			...
			return toGeometry(new Envelope(cs.getX(0), cs.getX(1), cs.getY(0),
					cs.getY(1)));
		}
	...
	public void encodeBox(Polygon poly,StringBuffer buff,Map<String,String> abstractGeometryAttributeMap) {
		encodeStartTag("Box", buff,abstractGeometryAttributeMap);
		Envelope env = poly.getEnvelopeInternal();
		Coordinate[] array = new Coordinate[]{ new Coordinate(env.getMinX(),env.getMinY()), new Coordinate(env.getMaxX(),env.getMaxY())};
		CoordinateSequence sequence = csFactory.create(array);
		encodeCoordinates(sequence, buff);
		encodeEndTag("Box", buff);		
	}
	
	public void encodeLineString(LineString ls,StringBuffer buff,Map<String,String> abstractGeometryAttributeMap) {
	...
	public void encodeLinearRing(LinearRing lr,StringBuffer buff,Map<String,String> abstractGeometryAttributeMap) {
	...
	public void encodePoint(Point p, StringBuffer buff,Map<String,String> abstractGeometryAttributeMap) {
	...
	public void encodePolygon(Polygon poly, StringBuffer buff,Map<String,String> abstractGeometryAttributeMap) {
	...
	public void encodeMultiPoint(MultiPoint mp, StringBuffer buff,Map<String,String> abstractGeometryAttributeMap) {
	...
	public void encodeMultiLineString(MultiLineString mls, StringBuffer buff,Map<String,String> abstractGeometryAttributeMap) {
	...
	public void encodeMultiPolygon(MultiPolygon mp, StringBuffer buff,Map<String,String> abstractGeometryAttributeMap) {
