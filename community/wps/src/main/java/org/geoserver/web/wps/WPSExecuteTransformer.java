...
import org.geoserver.web.wps.InputParameterValues.ParameterValue;
...
/**
 * Helper class to turn a {@link ExecuteRequest} into the corresponding WPS 1.0
 * Execute xml
 * 
...
class WPSExecuteTransformer extends TransformerBase {
	...
	public class ExecuteRequestTranslator extends TranslatorSupport {
		...
		public void encode(Object o) throws IllegalArgumentException {
			...
			AttributesImpl attributes = attributes("version", "1.0.0",
					"service", "WPS", "xmlns:xsi", XSI_URI, "xmlns", WPS_URI,
					"xmlns:wfs", WFS_URI, "xmlns:wps", WPS_URI, "xmlns:ows",
					OWS.NAMESPACE, "xmlns:gml", GML.NAMESPACE, "xmlns:ogc",
					OGC.NAMESPACE, "xmlns:wcs", WCS_URI, "xmlns:xlink",
...
//    			}
		...
		private void handleVectorInput(ParameterValue value) {
			...
			if (vector.layerName == null) {
				...
				start("wfs:Query", attributes("typeName",
						vector.layerName));
			}
		...
		private void handleTextInput(ParameterValue value) {
			start("wps:Data");
			start("wps:ComplexData", attributes("mimeType",
					value.mime));
			String data = Converters.convert(value.value,
					String.class);
			if (data != null) {
		...
		private Document parseAsXML(String data) {
			try {
				...
				LOGGER.log(Level.INFO,
						"Failed to parse XML, assuming it's plain text", t);
				return null;
			}
		}
		...
		public void handleOutputs(List<OutputParameter> outputs) {
			...
			if (outputs.size() > 1) {
				...
				for (OutputParameter op : outputs) {
					if (op.isComplex()) {
						if (op.isComplex()) {
							start("wps:Output", attributes("mimeType",
									op.mimeType));
						} else {
							start("wps:Output");
						}
				...
				if (op.isComplex()) {
					start("wps:RawDataOutput", attributes("mimeType",
							op.mimeType));
				} else {
