...
public class DeleteSuite extends TransactionSuite {
	...
	/*
	 * private static boolean runKvpTest(TransactionRequest baseRequest, String
	 * requestString, boolean match) throws Exception { // Read the file and
	 * parse it DeleteKvpReader reader = new DeleteKvpReader(requestString);
	 * TransactionRequest request = reader.getRequest(); LOGGER.finer("base
	 * request: " + baseRequest); LOGGER.finer("read request: " + request);
	 * LOGGER.fine("KVP test passed: " + baseRequest.equals(request)); //
	 * Compare parsed request to base request if (match) { //return
	 * baseRequest.equals(request); return baseRequest.equals(request); } else {
	...
	/*
	 * Need updated geotools jar...big fix takes care of this problem The fix is
	 * in cvs right now, hopefully release will come soon.
	 */
