...
public abstract class RequestTestCase extends TestCase {
	...
	private static final String DATA_DIRECTORY = new StringBuffer(System
			.getProperty("user.dir")).append("/test/test-data/requests/")
			.toString();

	/** Holds mappings between HTTP and ASCII encodings */
	protected static FilterFactory factory = FilterFactoryFinder
			.createFilterFactory();

	/** Unit test data directory */
	private static final String CONFIG_DIR = new StringBuffer(System
			.getProperty("user.dir")).append("/misc/unit/config/").toString();
	 ...
	 * 
	 * @throws Exception
	 *             If there is any problem running the test.
	 */
	protected boolean runXmlTest(Request baseRequest, String fileName,
			boolean match) throws Exception {
		// Read the file and parse it
		File inputFile = new File(new StringBuffer(DATA_DIRECTORY).append("/")
	...
	protected boolean runKvpTest(Request baseRequest, String requestString,
		...
		Request request = reader
				.getRequest(baseRequest.getHttpServletRequest());
