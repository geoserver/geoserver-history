...
public class SVGWriter extends OutputStreamWriter {
	private static final Logger LOGGER = Logger.getLogger(SVGWriter.class
			.getPackage().getName());

	/**
	 * a number formatter setted up to write SVG legible numbers ('.' as decimal
	...
	public void setGeometryType(Class gtype) {
		 ...
		 * SVGFeatureWriterHandler(); this.writerHandler = new
		 * FIDSVGHandler(this.writerHandler); this.writerHandler = new
		 * BoundsSVGHandler(this.writerHandler); this.writerHandler = new
	...
	public void writeFeatures(FeatureReader reader, String style)
		...
		try {
			...
			/*
			 * boolean doCollect = config.isCollectGeometries() && (gtype !=
			 * Point.class) && (gtype != MultiPoint.class);
			 */
	...
	private void setUpWriterHandler(FeatureType featureType, boolean doCollect)
		...
		if (doCollect) {
			...
			/*
			 * REVISIT: get rid of all this attribute stuff, since if attributes
			 * are needed it fits better to have SVG with gml attributes as
			 * another output format for WFS's getFeature.
			 */
			...
			if (atts.size() > 0) {
				this.writerHandler = new AttributesSVGHandler(
						this.writerHandler);
	...
	public class AttributesSVGHandler extends SVGFeatureWriterHandler {
		 ...
		 * <code>c</code>, such as <code>"&#x00d1;"</code> for the
		 * character <code>'?'</code>
		 * 
		 * @param c
		 *            DOCUMENT ME!
	...
	private abstract class SVGFeatureWriter {
		 ...
		 * <code>minCoordDistance</code> and established by the method
	...
	private class PointWriter extends SVGFeatureWriter {
		 ...
		 * else { write("point"); } write("\""); } super.writeAttributes(ft); }
		 * 
		 * @throws IOException
		 *             DOCUMENT ME!
		 */
	...
	/**
	 * Writer to handle feature types which contain a Geometry attribute that is
	 * actually of the class Geometry. This can occur in heterogeneous data
	 * sets.
	 * 
	...
	private class PolygonWriter extends SVGFeatureWriter {
		...
		protected void writeGeometry(Geometry geom) throws IOException {
			...
			for (int i = 0; i < nHoles; i++)
				writeClosedPathContent(poly.getInteriorRingN(i)
						.getCoordinates());
		}
	}
