 ...
 * <code>http://www.keypoint.com/</code> who let me develop this code on
 * company time.
 * </p>
 * 
 * <p>
 ...
 * 
 * CHANGES: -------- 19-Sep-2003 : Fix for platforms using EBCDIC (contributed
 * by Paulo Soares); 19-Oct-2003 : Change private fields to protected fields so
 * that PngEncoderB can inherit them (JDE) Fixed bug with calculation of nRows
 * Added modifications for unsigned short images (contributed by Christian at
 * xpogen.com)
 */
...
public class PngEncoderB extends PngEncoder {
	...
	protected void writeHeader() {
		...
		if (bytesPerPixel != 1) {
			bytePos = writeByte((encodeAlpha) ? 6 : 2, bytePos); // direct
																	// model
		} else {
	...
	/**
	 * assume -- write alpha -- 4 byte colour model -- no filter
	 * 
	 * @return
	 */
	protected boolean writeImageData_optimized() {
		...
		ByteArrayOutputStream outBytes = new ByteArrayOutputStream(20000); // made
																			// bufferer
																			// bigger
																			// (we'll
																			// get
		...
		try {
			while (rowsLeft > 0) {
				...
				for (int i = 0; i < nRows; i++) {
					...
					System.arraycopy(pixels, readPos, scanLines, scanPos,
							4 * width);
					readPos += 4 * width;
					scanPos += 4 * width;
				}
