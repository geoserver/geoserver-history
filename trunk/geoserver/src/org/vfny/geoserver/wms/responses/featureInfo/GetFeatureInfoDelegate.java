 ...
 * 
 * <p>
 * Subclasses should implement one or more output formats, wich will be returned
 * in a list of mime type strings in <code>getSupportedFormats</code>. For
 * example, a subclass can be created to write one of the following output
 * formats:
 * 
 * <ul>
 * <li> text/plain </li>
 ...
 * y, etc., create the gt2 query objects for each featuretype and executing it.
 * This process leads to a set of FeatureResults objects and its metadata, wich
 * will be given to the <code>execute(FeatureTypeInfo[] ,
 * FeatureResults[])</code>
 * method, that a subclass should implement as a matter of setting up any
 ...
 * @author Chris Holmes
 * @version $Id: GetFeatureInfoDelegate.java,v 1.1 2004/07/15 21:13:14 jmacgill
 *          Exp $
 */
public abstract class GetFeatureInfoDelegate implements Response {
	...
	/**
	 * Executes a Request, which must be a GetMapRequest. Any other will cause a
	 * class cast exception.
	 * 
	 * @param request
	 *            A valid GetMapRequest.
	 * 
	...
	/**
	 * Execute method for concrete children to implement. Each param is an array
	 * in the order things should be processed.
	 * 
	 * @param requestedLayers
	 *            Array of config information of the FeatureTypes to be
	 *            processed.
	 * @param queries
	 *            Matching array of queries from the queries of the requested
	 *            layers, already setted up with the bbox filter from the BBOX
	 *            parameter of the GetMap request and to retrieve the specified
	 *            attributes in the ATTRIBUTES request parameter.
	 * @param x
	 *            the X coordinate in pixels where the identification must be
	 *            done relative to the image dimensions
	 * @param y
	 *            the Y coordinate in pixels where the identification must be
	 *            done relative to the image dimensions
	 * 
	 * @throws WmsException
	 *             For any problems executing.
	 */
	 ...
	 * 
	 * @param layers
	 *            The layers to request against.
	 * 
	 * @return An array of queries, matching the arrays passed in.
	 * 
	 * @throws WmsException
	 *             If the custom filter can't be constructed.
	 */
	private Query[] buildQueries(FeatureTypeInfo[] layers) throws WmsException {
		...
		try {
			...
			throw new WmsException(ex, "Can't build layer queries: "
					+ ex.getMessage(), getClass().getName() + "::parseFilters");
	 ...
	 * 
	 * @param requestExtent
	 *            The extent to filter out.
	 * @param ffactory
	 *            A filterFactory to create new filters.
	 * @param schema
	 *            The FeatureTypeInfo of the request of this filter.
	 * 
	 * @return A custom filter of the bbox and any optional custom filters.
	 * 
	 * @throws IllegalFilterException
	 *             For problems making the filter.
	 */
	 ...
	 *       the method to find the rest of attributes needed by inspecting the
	 *       filter (would be enough to get all the AttributeExpression's?). I
	 *       think that the style should be taken in count too.
	 */
