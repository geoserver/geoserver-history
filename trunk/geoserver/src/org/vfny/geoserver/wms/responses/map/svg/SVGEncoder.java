...
public class SVGEncoder {
	 ...
	 * coordinates to an SVG viewbox space. It is necessary due to the different
	 * origin of Y coordinates in SVG space and in Coordinates space
	 * 
	 ...
	 * @uml.associationEnd elementType="com.vividsolutions.jts.geom.Geometry"
	 *                     multiplicity= "(0 -1)"
	 */
	 ...
	 * writting of attributes and ids. It is usefull to significantly reduce the
	 * size of the resulting SVG content when no other data than the graphics is
	 * needed for a single layer
	 */
	...
	/**
	 * If <code>collect == true</code>, then all the geometries will be
	 * grouped in a single SVG element by FeatureTypeInfo requested. The effect
	 * is like a union operation upon the geometries of the whole FeatureResults
	 ...
	 * 
	 * @param collect
	 *            wether to collect geometries into a single svg element for
	 *            each FeatureResults
	 */
	...
	/**
	 * sets the "viewBox" of the generated SVG and establishes the encoding blur
	 * factor to <code>blurFactor</code>
	 * 
	 * <p>
	 ...
	 * against the minimun coordinate separation distance as the easyest way of
	 * keeping polygon shapes consistent
	 * </p>
	 * 
	 * <p>
	 * NOTE: if you don't want that klind of hacky geometry generalyzation, just
	 * pass <code>0</code> (zero) as <code>blurFactor</code>
	 * </p>
	 * 
	 * @param env
	 *            DOCUMENT ME!
	 ...
	 * 
	 * @param layers
	 *            the outermost SVG<code>g</code> element id wich will group
	 *            all the features in the collection
	 * @param results
	 *            the features to be encoded as SVG vectors
	 * @param styles
	 *            DOCUMENT ME!
	 * @param out
	 *            SVG encoder output stream
	 * 
	 * @throws IOException
	 *             DOCUMENT ME!
	 */
	...
	private void writeLayers(FeatureTypeInfo[] layers,
			FeatureResults[] results, Style[] styles) throws IOException,
			AbortedException {
		int nLayers = results.length;
		int nConfigs = ((layers != null) && (layers.length >= nLayers)) ? nLayers
				: 0;
	...
	private void writeDefs(FeatureTypeInfo[] layers) throws IOException {
		if (layers == null) {
			LOGGER
					.warning("Can't write symbol definitions, no FeatureTypes passed");
		...
		for (int i = 0; i < nLayers; i++) {
			Class geometryClass = layers[i].getFeatureType()
	...
	private String createViewBox() {
		String viewBox = (long) getX(referenceSpace.getMinX())
				+ " "
				+ (long) (getY(referenceSpace.getMinY()) - referenceSpace
						.getHeight()) + " " + (long) referenceSpace.getWidth()
	...
	private void ensureSVGSpace(FeatureResults[] results) throws IOException {
		if (this.referenceSpace == null) {
			...
			LOGGER
					.fine("no explicit SVG space defined, using bounds "
							+ bounds);
	 ...
	 *             DOCUMENT ME!
	 */
	private void writeFeatures(FeatureReader reader) throws IOException,
			AbortedException {
		Feature ft;
	 ...
	 * method performs a kind of very basic path generalization, verifying that
	 * the distance between the current coordinate and the last encoded one is
	 * greater than the minimun distance expressed by the field
	 ...
	 * 
	 * @throws IOException
	 *             si algo ocurre escribiendo a <code>out</code>
	 */
		...
		for (int i = 0; i < numAtts; i++) {
			...
			if (!(att instanceof Geometry)) {
				writer.writeAttribute(
						featureType.getAttributeType(i).getName(), att);
			}
		}
	}
	...
	/**
	 * if a reference space has been set, returns a translated Y coordinate wich
	 * is inverted based on the height of such a reference space, otherwise just
	 * returns <code>y</code>
	 * 
	 * @param y
	 *            DOCUMENT ME!
