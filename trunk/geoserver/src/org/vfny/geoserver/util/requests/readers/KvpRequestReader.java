...
abstract public class KvpRequestReader {
	...
	/**
	 * Attempts to parse out the proper typeNames from the FeatureId filters. It
	 * simply uses the value before the '.' character.
	 * 
	 * @param rawFidList
	...
	protected static List getTypesFromFids(String rawFidList)
		...
		while (i.hasNext()) {
			 ...
			 ids = (List) i.next();
			 innerIterator = ids.listIterator();
			while (innerIterator.hasNext()) {
				fid = innerIterator.next().toString();
				if (LOGGER.isLoggable(Level.FINER)) {
					LOGGER.finer(new StringBuffer("looking at featureId")
							.append(fid).toString());
				}

				pos = fid.indexOf(".");
				typeName = fid.substring(0, pos);
				if (LOGGER.isLoggable(Level.FINER)) {
					LOGGER.finer(new StringBuffer("adding typename: ").append(
							typeName).append(" from fid").toString());
				}
					...
					if (LOGGER.isLoggable(Level.FINEST)) {
						LOGGER
								.finest(new StringBuffer(
										"adding simple element ").append(token)
										.toString());
					}
	...
	protected static List readNested(String rawList) throws WfsException {
		if (LOGGER.isLoggable(Level.FINEST)) {
			LOGGER.finest(new StringBuffer("reading nested: ").append(rawList)
					.toString());
		}
	 ...
	 * 
	 * @param fid
	 *            A group of feature IDs, as a String.
	 * @param filter
	 *            A group of filters, as a String.
	 * @param bbox
	 *            A group of boxes, as a String.
	 * 
	...
	protected static List readFilters(String fid, String filter, String bbox)
		...
		if ((fid != null) && (filter == null) && (bbox == null)) {
			if (LOGGER.isLoggable(Level.FINEST)) {
				LOGGER.finest(new StringBuffer("reading fid filter: ").append(
						fid).toString());
			}
			...
			while (i.hasNext()) {
				...
				while (innerIterator.hasNext()) {
					...
					if (LOGGER.isLoggable(Level.FINEST)) {
						LOGGER.finest(new StringBuffer("added fid filter: ")
								.append(fidFilter).toString());
					}
				}
			}
			...
			if (LOGGER.isLoggable(Level.FINEST)) {
				LOGGER.finest(new StringBuffer("reading filter: ").append(
						filter).toString());
			}
			...
			Reader filterReader;
			while (i.hasNext()) {
				filterReader = new StringReader((String) i.next());
			...
			if (LOGGER.isLoggable(Level.FINEST)) {
				LOGGER.finest(new StringBuffer("bbox filter: ").append(bbox)
						.toString());
			}
			...
			throw new WfsException("GetFeature KVP request contained "
					+ "conflicting filters.  Filter: " + filter + ", fid: "
					+ fid + ", bbox:" + bbox);
		}
	}
	...
	public static Map parseKvpSet(String qString) {
		...
		if (LOGGER.isLoggable(Level.FINE)) {
			LOGGER.fine(new StringBuffer("clean request is ").append(
					cleanRequest).toString());
		}
		...
		while (requestKeywords.hasMoreTokens()) {
			 kvpPair = requestKeywords.nextToken();
			...
			if (kvpPair.toUpperCase().startsWith("FILTER")) {
				 filterVal = kvpPair.substring(7);

				// int index = filterVal.lastIndexOf("</Filter>");
				// String filt2 = kvpPair.subString
				if (LOGGER.isLoggable(Level.FINEST)) {
					LOGGER.finest(new StringBuffer("putting filter value ")
							.append(filterVal).toString());
				}
		...
		if (LOGGER.isLoggable(Level.FINE)) {
			LOGGER.fine(new StringBuffer("returning parsed ").append(kvps)
					.toString());
		}
	...
	private static String clean(String raw) {
		if (LOGGER.isLoggable(Level.FINEST)) {
			LOGGER.finest(new StringBuffer("raw request: ").append(raw)
					.toString());
		}
		...
		if (raw != null) {
			try {
				...
				if (LOGGER.isLoggable(Level.FINER)) {
					LOGGER.finer(new StringBuffer("Bad encoding for decoder ")
							.append(e).toString());
				}
			}
		} else {
		...
		if (LOGGER.isLoggable(Level.FINEST)) {
			LOGGER.finest(new StringBuffer("cleaned request: ").append(raw)
					.toString());
		}
