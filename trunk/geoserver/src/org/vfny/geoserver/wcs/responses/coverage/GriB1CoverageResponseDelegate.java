...
public class GriB1CoverageResponseDelegate implements CoverageResponseDelegate {
	...
	public boolean canProduce(String outputFormat) {
		if (outputFormat.equalsIgnoreCase("GriB1"))
			...
			// //As an instance if we were using an Ascii Grid coverage we would
			// not be able (FOR THE MOMENT)
			...
			// GRIB1Writer("fake!!").getMetadataNames();
	...
	public void encode(OutputStream output) throws ServiceException,
		...
		final GridCoverageWriter writer = new GRIB1Writer(output);
		if (writer != null) {
			...
			if (metadataNames != null) {
				...
				for (int j = 0; j < length; j++)
					writer.setMetadataValue(metadataNames[j], sourceCoverage
							.getMetadataValue(metadataNames[j]));
				// write
		...
		throw new ServiceException(
				"Could not instantiate a grib writer. Reason might be missing metadata! (This is a known issue!)");
	}
