 ...
 * exception that wfs clients will expect. All errors should be wrapped in this
...
public class WcsException extends ServiceException {
	...
	public String getXmlResponse(boolean printStackTrace,
		...
		// Perhaps not the best place to do this, but it's by far the best place
		// to ensure
		// that all logged errors get recorded in the same way, as there all
		// must return
		// xml responses.
		...
		returnXml.append(indent).append("version=\"1.2.0\"\n");

		returnXml.append(indent).append(
		...
		returnXml.append(Requests.getSchemaBaseUrl(request)).append(
				"wcs/1.0.0/OGC-exception.xsd\">\n");

		// REVISIT: handle multiple service exceptions? must refactor class.
		returnXml.append(indent).append("<ServiceException");

		if (!isEmpty(this.code)) {
			returnXml.append(" code=\"").append(this.code + "\"");
		}

		if (!isEmpty(this.locator)) {
			returnXml.append(" locator=\"").append(this.locator + "\"");
		}
