...
public class GeoServer extends GlobalLayerSupertype {
	 ...
	 * reference, as GeoServer by default puts up the schemas that it needs and
	 * references them. It could be used to specify an alternate site for the
	 * schemas, however, for example if a user didn't want their servlet
	 ...
	 *       method can do the local referencing themselves. For now no one is
	 *       using this method, perhaps we should just leave it out for 1.2.0,
	 *       as it's very obscure. I think I only added it originally because I
	 *       didn't want to go through the busy work of cleaning up and figuring
	 *       out how to copy over the ogc schemas.
	 */
	 ...
	 *             If an error occurs
	 */
	public void load(GeoServerDTO dto, ServletContext context)
			throws ConfigurationException {
		if (dto != null) {
	 ...
	 * 
	 * @param location
	 *            The log file path, this can be an absolute or relative path.
	 * @param context
	 *            The servlet context
	 * 
	 ...
	 * 
	 */
	public static void initLogging(Level level, boolean logToFile,
			String location, ServletContext context) throws IOException {
	 ...
	 * not reference an actual physical location on disk. Call
	 * {@link GeoServer#getLogLocation(String, ServletContext)} to map this
	...
	/**
	 * @param logLocation
	 *            The string representation of the path on disk in which the
	 *            server logs to.
	 */
