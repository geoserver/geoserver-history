...
public class GeoServerTemplateLoader implements TemplateLoader {
	...
	public GeoServerTemplateLoader( Class caller ) throws IOException {
		...
		fileTemplateLoader = 
			new FileTemplateLoader( GeoserverDataDirectory.getGeoserverDataDirectory() );
	...
	public Object findTemplateSource(String path) throws IOException {
		...
		if ( featureType != null ) {
			template = (File) fileTemplateLoader.findTemplateSource(
				"featureTypes" + File.separator + featureType + 
					File.separator + path 
			);
			if ( template != null ) {
				return template;
		...
		if ( source instanceof File ) {
			...
			try {
				File templateFile = new File( 
					GeoserverDataDirectory.findConfigFile( "templates" ), wrapper.path
				);
