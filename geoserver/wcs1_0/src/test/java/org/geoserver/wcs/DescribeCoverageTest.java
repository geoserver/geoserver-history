...
//        print(dom);
    ...
    public void testDescribeDemCoverageKvp() throws Exception {
        Document dom = getAsDOM(BASEPATH + "?request=DescribeCoverage&service=WCS&version=1.0.0&coverage=" + layerId(WCSTestSupport.TASMANIA_DEM));
//        print(dom);
    ...
    public void testDescribeDemCoverageXml() throws Exception {
        ...
        String request = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\r\n" + // 
            ...
            "  xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" \r\n" + // 
            "  version=\"1.0.0\" >\r\n" + //
            "  <wcs:Coverage>" + layerId(WCSTestSupport.TASMANIA_DEM) + "</wcs:Coverage>\r\n" + // 
            "</wcs:DescribeCoverage>";
        Document dom = postAsDOM(BASEPATH, request, errors);
        checkValidationErrors(dom, WCS10_SCHEMA);
        checkDemCoverageDescription(dom);
    ...
    private void checkDemCoverageDescription(Document dom) throws Exception {
        ...
        Node gridOffsets = xpath.getMatchingNodes("/wcs:CoverageDescription/wcs:CoverageOffering/" +
                "wcs:domainSet/wcs:spatialDomain/gml:RectifiedGrid/gml:offsetVector", dom).item(0);
        String[] offsetStrsLow = gridOffsets.getTextContent().split(" ");
        gridOffsets = xpath.getMatchingNodes("/wcs:CoverageDescription/wcs:CoverageOffering/" +
                "wcs:domainSet/wcs:spatialDomain/gml:RectifiedGrid/gml:offsetVector", dom).item(1);
    ...
    public void testDescribeRotatedCoverage() throws Exception {
        Document dom = getAsDOM(BASEPATH + "?request=DescribeCoverage&service=WCS&version=1.0.0&coverage=" + layerId(WCSTestSupport.ROTATED_CAD));
//        print(dom);
        checkValidationErrors(dom, WCS10_SCHEMA);
        // check the basics, the output is a single coverage description with the expected id
        assertEquals(1, dom.getElementsByTagName("wcs:CoverageDescription").getLength());
