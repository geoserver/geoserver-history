...
public class RewindableInputStream extends InputStream {
    /**
     * Default buffer size before we've finished with the XMLDecl: I
     * think the name should be left unchanged to give a hint for possible use
     * of this class :)
     */
    public static final int DEFAULT_XMLDECL_BUFFER_SIZE = 64;

    /**
     * Tells whether <code>read(byte[], int, int)</code> method is
     * allowed to read multiple bytes beyond the internal buffer
     ...
     * Potentially in <code>RewindableInputStream</code> instance stream's
     * "end" could be reached more than once, so it is a good thing to know
     * where original stream ended to avoid <code>IOExceptions</code>.
     */
    ...
    /**
     * Number of read bytes currently stored in <code>fData</code>
     * buffer. Size of the buffer itself can be greater than this value,
     * obviously.
     */
    protected int fLength;

    /**
     * Offset of the "marked" position in the stream relative to its
     * beginning.
     */
    protected int fMark;
    ...
    /**
     * Sets the position somewhere in the stream to which the stream
     * pointer will be reset after <code>rewind</code> invocation. By default
     * this position is the beginning of the stream.
     *
    ...
    /**
     * Allows to change the behavior of the stream regarding chunked
     * reading at runtime. If you allowed chunked reading and then read some
     * data from the stream, you better forget about <code>reset</code>ting or
    ...
    /**
     * More conscious alias for <code>setChunkedMode(true)</code>.
     * While last method is a general purpose mutator, code may look a bit
     * more clear if you use specialized methods to enable/disable chunk read
     * mode.
     */
    ...
    /**
     * More conscious alias for <code>setChunkedMode(false)</code>.
     * While last method is a general purpose mutator, code may look a bit
     * more clear if you use specialized methods to enable/disable chunk read
     * mode.
     */
    ...
    /**
     * Quickly reset stream pointer to the beginning of the stream or
     * to position which offset was specified during the last
    ...
    /**
     * Reads next byte from this stream. This byte is either being read
     * from underlying InputStream or taken from the internal buffer in case
     * it was already read at some point before.
     *
     * @return Next byte of data or <code>-1</code> if end of stream is
     *         reached.
     *
     ...
     * array of bytes. In its current implementation it cannot return more
     * bytes than left in the buffer if in "non-chunked" mode
     * (<code>fMayReadChunks == false</code>). After reaching the end of the
     * buffer, each invocation of this method will read exactly 1 byte then.
     * In "chunked" mode this method <em>may</em> return more than 1 byte, but
     * it doesn't buffer the result.<p>From the other hand, for the
     * task of reading xml declaration, such behavior may be desirable, as we
     * probably don't need reset/rewind functionality after we finished with
     * charset deduction. It is good idea to call
     * <code>enableChunkedMode</code> after that, in order to improve
     * perfomance and lessen memoery consumption when reading the rest of the
     * data.</p>
     *
     * @param b DOCUMENT ME!
     ...
     *
     * @return Total number of bytes actually read or <code>-1</code> if end of
     ...
     * skipping over some smaller number of bytes, possibly <code>0</code>.
     ...
     * for this input stream. For <code>RewindableInputStream</code> this can
     * be:
     *  <ul>
     *      <li>Number of unread bytes in the <code>fData</code>
     *      buffer, i.e. those between current position (fOffset) and total
     *      bytes quantity in the buffer (fLength).</li>
     *      <li>Result of underlying InputStream's
     *      <code>available</code> call if there are no unread bytes in the
     *      buffer.</li>
     *      <li><code>-1</code> if end of stream is reached.</li>
     *  </ul>
     *
     ...
     * using <code>mark</code> method (or to beginning of the stream, if there
     * were no <code>mark</code> method calls).
     */
    public void reset() {
        fOffset = fMark;
    }
    ...
    /**
     * Tells that this stream supports mark/reset capability. This one
