 ...
 * <p>
 * This class implements {@link org.geotools.catalog.Service} interface as a link to a catalog.
 * </p>
 * 
...
public class FeatureTypeInfo extends GlobalLayerSupertype {
    ...
    //
    // /**
    // * Should we be adding the CacheControl: max-age header to outgoing maps which include this
    // layer?
    // */
    ...
    public FeatureTypeInfo(LayerInfo layer, Catalog catalog) {
        ...
        if (layer != null)
            ...
            featureType = new DummyFeatureTypeInfo();
    }
     ...
     * 
     * @param dto
     *                FeatureTypeInfoDTO The data to populate this class with.
     * @param data
     *                Data a reference for future use to get at DataStoreInfo instances
     * 
    ...
    public void load(FeatureTypeInfoDTO dto) throws Exception {
        ...
        setSRS(dto.getSRS());
        featureType.setLatLonBoundingBox(new ReferencedEnvelope(dto.getLatLongBBox(),
        ...
        if (dto.getSchemaAttributes() != null) {
            for (Iterator i = dto.getSchemaAttributes().iterator(); i.hasNext();) {
                ...
                org.geoserver.catalog.AttributeTypeInfo ati = catalog.getFactory()
                        .createAttribute();
        ...
        featureType.setNativeBoundingBox(new ReferencedEnvelope(dto.getNativeBBox(), featureType
                .getCRS()));
     ...
     * 
     * <p>
     * This method is package visible only, and returns a reference to the GeoServerDTO. This method
     * is unsafe, and should only be used with extreme caution.
     * </p>
     * 
     ...
     * 
     * <p>
     * gets the string of the path to the schema file. This is set during feature reading, the
     * schema file should be in the same folder as the feature type info, with the name schema.xml.
     * This function does not guarantee that the schema file actually exists, it just gives the
    ...
    /**
     * By now just return the default style to be able to declare it in WMS capabilities, but all
     * this stuff needs to be revisited since it seems currently there is no way of retrieving all
     * the styles declared for a given SimpleFeatureType.
     * 
    ...
    /**
     * Indicates if this FeatureTypeInfo is enabled. For now just gets whether the backing datastore
     * is enabled.
     * 
     * @return <tt>true</tt> if this FeatureTypeInfo is enabled.
     * 
     * @task REVISIT: Consider adding more fine grained control to config files, so users can
     *       indicate specifically if they want the featureTypes enabled, instead of just relying on
     *       if the datastore is. Jody here - this should be done on a service by service basis WMS
     *       and WFS will need to decide for themselves on this one
     */
     ...
     * <p>
     * This isn't _really_ necessary, but I'm putting it in in case we change namespaces, letting
     * FeatureTypes set their own namespaces instead of being dependant on datasources. This method
     * will allow us to make that change more easily in the future.
     * 
     ...
     */
    public FeatureSource<SimpleFeatureType, SimpleFeature> getFeatureSource(boolean skipReproject)
            throws IOException {
    ...
    /**
     * Returns the SimpleFeatureType's envelope in its native CRS (or user declared CRS, if any).
     * <p>
     * Note the Envelope is cached in order to avoid a potential performance penalty every time this
     * value is requires (for example, at every GetCapabilities request)
     * </p>
     * 
     ...
     * <p>
     * This is the typeName as provided by the gt2 datastore, unless an alias is set, in that case
    ...
    // /**
    // * Fixes the data store feature type so that it has the right CRS (only in case they are
    // missing)
    ...
    // }
    // } catch (Exception e) {
    // e.printStackTrace(); //DJB: this is okay to ignore since (a) it should never happen (b) we'll
    // use the default one (crs=null)
    // }
    // }
    //
     ...
     * 
     * <p>
     * Convience method for accessing attribute names as a Collection. You may use the names for
     * AttributeTypeMetaData lookup or with the schema for XPATH queries.
     * </p>
     * 
     * @return List of attribute names
     * 
     * @task REVISIT: This method sucks. It didn't do the same thing as getAttributes, which it
     *       should have. I fixed the root problem of why attribs.size() would equal 0. So the
     *       second half of this method should probably be eliminated, as it should never be called.
     *       But I don't want to break code right before a release - ch.
     * 
    ...
    /**
     * Returns a list of the attributeTypeInfo objects that make up this SimpleFeatureType.
     * 
    ...
    /**
     * Gets the schema.xml file associated with this SimpleFeatureType. This is set during the
     * reading of configuration, it is not persisted as an element of the FeatureTypeInfoDTO, since
     * it is just whether the schema.xml file was persisted, and its location. If there is no
     * schema.xml file then this method will return a File object with the location where the schema
     ...
     * 
     * @return a string representing the number of seconds to be added to the "Cache-Control:
     *         max-age=" header
     */
     ...
     * 
     * @param cacheMaxAge
     *                a string representing the number of seconds to be added to the "Cache-Control:
     *                max-age=" header
     */
     ...
     *                true if we should add the header, false if we should omit the header
     */
     ...
     *                true if we should, false if we should not list it
     */
    ...
    /**
     * Returns the maximum number of features to be served by WFS GetFeature for this feature type
     * (or 0 for no limit)
     * 
     * @return
     */
    ...
    /**
     * DummyFeatureTypeInfo to be used when there is no feature type in the configuration for the
     * layer being processed, such as when an SLD with inline features is being processed.
     * 
     * @author David Winslow, OpenGeo (not really, I just autogenerated it with eclipse. But it's
     *         dumb code so I thought I'd put a name on it.)
     */
    private static class DummyFeatureTypeInfo implements org.geoserver.catalog.FeatureTypeInfo {
        ...
        public Map<String, Serializable> getMetadata() {
            return (Map<String, Serializable>) Collections.EMPTY_MAP;
        }
