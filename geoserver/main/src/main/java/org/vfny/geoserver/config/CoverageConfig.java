...
public class CoverageConfig {
	...
	/**
	 * Storing here all the valid time positions or time period for this
	 * Coverage
	 */
	 ...
	 */
	public CoverageConfig(String formatId, Format format,
			AbstractGridCoverageNDReader reader, String coverageName,
		...
		// BANDS (RangeSet)
		// //
		final String[] bandKeys = reader.getMetadataValue("band_keys",
				coverageName).split(", ");
		...
		for (int dim=0; dim < dimensions.length; dim++) {
			...
			// //
			// TODO: Add real Range and NullValues as soon they will be
			// available from ImageMetadata (still on development right now)
			// //
		...
		if (crs != null) {
			...
			for (int i = 0; i < dimentionNames.length; i++) {
				dimentionNames[i] = new SimpleInternationalString(cs.getAxis(i)
						.getName().getCode());
			}
		} else {
			dimentionNames = (InternationalString[]) XArray.resize(
					DIMENSION_NAMES, crs.getCoordinateSystem().getDimension());

			for (int i = DIMENSION_NAMES.length; i < dimentionNames.length; i++) {
				dimentionNames[i] = new SimpleInternationalString("dim"
						+ (i + 1));
			}
		}
		...
		description = new StringBuffer(name).append(" [").append(label).append(
				"] has been generated from ").append(formatId).toString();
		...
		if ((reader.getCrs(coverageName).getIdentifiers() != null)
			...
			requestCRSs.add(((Identifier) reader.getCrs(coverageName)
		...
		if ((reader.getCrs(coverageName).getIdentifiers() != null)
				&& !reader.getCrs(coverageName).getIdentifiers().isEmpty()) {
			responseCRSs.add(((Identifier) reader.getCrs(coverageName)
					.getIdentifiers().toArray()[0]).toString());
		}
	...
	public CoverageConfig(String formatId, Format format,
			AbstractGridCoverage2DReader reader, HttpServletRequest request)
			throws ConfigurationException {
		if ((formatId == null) || (formatId.length() == 0)) {
			throw new IllegalArgumentException(
					"formatId is required for CoverageConfig");
		...
		try {
			...
			final GeneralEnvelope subEnvelope = new GeneralEnvelope(minCP,
					maxCP);
			subEnvelope.setCoordinateReferenceSystem(reader.getCrs());

			parameters.put(AbstractGridFormat.READ_GRIDGEOMETRY2D.getName()
					.toString(), new GridGeometry2D(reader
					.getOriginalGridRange(), subEnvelope));
			gc = (GridCoverage2D) reader.read(CoverageUtils.getParameters(
					readParams, parameters, true));
	...
	private CoverageDimension[] parseCoverageDimesions(
		...
		for (int i = 0; i < length; i++) {
			...
			for (Iterator c_iT = categories.iterator(); c_iT.hasNext();) {
				...
				if ((cat != null)
						&& cat.getName().toString().equalsIgnoreCase("no data")) {
					double min = cat.getRange().getMinimum();
					double max = cat.getRange().getMaximum();

					dims[i]
							.setNullValues(((min == max) ? new Double[] { new Double(
									min) }
									: new Double[] { new Double(min),
											new Double(max) }));
				}
			}
