...
public class XMLConfigReader {
	...
	protected void loadCatalog(File catalogFile, File featureTypeDir,
		 ...
		 * "formats", true); data.setFormats(loadFormats(formatElement)); }
		 * catch (Exception e) { // It doesn't exist. This is ok, they are using
		 * 1.4 // We will just create the element for them and continue
		 * 
		 *  }
		 */
		try {
	...
	protected void loadGlobal(Element globalElem) throws ConfigurationException {
		try {
			...
			Element elem = null;
			elem = ReaderUtils.getChildElement(globalElem, "loggingToFile",
					false);
			...
			if (LOGGER.isLoggable(Level.CONFIG)) {
				LOGGER.config(new StringBuffer("numDecimals returning is ")
	...
	protected void loadWFS(Element wfsElement) throws ConfigurationException {
		...
		try {
			...
			if (LOGGER.isLoggable(Level.CONFIG)) {
				LOGGER.config(new StringBuffer("reading srsXmlStyle: ").append(
						elem).toString());
			}
