 ...
 * <code>org.apache.xerces.impl.XMLEntityManager</code>) returns an array with
 * name of encoding scheme and endianness. I decided to create a separate
 * class incapsulating encoding metadata. The may idea behind this is the fact
 * that we will most probably need to save this info somewhere and use it
 * later when writing the response. Beside that, using class makes related
 * code more clear.
 */
public class EncodingInfo {
    /**
     * This is a name of autodetected <em>encoding scheme</em> (not
     * necessarily <em>charset</em>) which should be used to read XML
     * declaration in order to determine actual data <em>charset</em>.
     */
     ...
     * <code>false</code> for little-endian, and <code>null</code> if byte
     * order is not relevant for this encoding scheme. This is a "three-state"
     * switch (third is <code>null</code>), so it can't be just plain
     ...
     * like characteristic of the input XML document. Tells whether Byte Order
     * Mark (BOM) was found while detecting encoding scheme.
     */
    ...
    /**
     * Accessor for <code>fIsBigEndian</code>. Should we define a
     * mutator too?
     *
     * @return DOCUMENT ME!
     */
     ...
     * instance. Strange enough, but sometimes such behavior is preferred to
     * simple assignment or cloning. More specifically, this method is
     * currently used (at least it was :) in
     * <code>XmlCharsetDetector.getCharsetAwareReader</code> (other two ways
     * simply don't work).
     *
