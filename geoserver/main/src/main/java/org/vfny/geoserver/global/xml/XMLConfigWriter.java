...
public class XMLConfigWriter {
	...
	public static void store(DataDTO data, File root)
		...
		try {
			Writer fw = new OutputStreamWriter(
					new FileOutputStream(catalogFile), getDefaultEncoding());
		...
		File coverageDir = WriterUtils.initFile(
				new File(dataDir, "coverages/"), true);
	...
	public static void store(WCSDTO wcs, WMSDTO wms, WFSDTO wfs,
			GeoServerDTO geoServer, File root) throws ConfigurationException {
		...
		try {
			Writer fw = new OutputStreamWriter(
					new FileOutputStream(configFile), getDefaultEncoding());
	...
	public static void store(WCSDTO wcs, WMSDTO wms, WFSDTO wfs,
			GeoServerDTO geoServer, DataDTO data, File root)
			throws ConfigurationException {
	 ...
	 */
	protected static void storeServices(WriterHelper cw, WCSDTO wcs,
			WMSDTO wms, WFSDTO wfs, GeoServerDTO geoServer)
			throws ConfigurationException {
		...
		if (g != null) {
			 ...
			 */
			cw
					.comment("Sets the max number of Features returned by GetFeature");
			...
			cw
					.comment("Whether the Service Exceptions returned to clients should contain\n"
							+ "full java stack traces (useful for debugging). ");
			cw.valueTag("verboseExceptions", "" + g.isVerboseExceptions());
			cw
					.comment("Sets the max number of decimal places past the zero returned in\n"
			...
			if ((g.getSchemaBaseUrl() != null) && (g.getSchemaBaseUrl() != "")) {
				cw
						.comment("Define a base url for the location of the wfs schemas.\n"
			...
			if ((g.getProxyBaseUrl() != null) && (g.getSchemaBaseUrl() != "")) {
				cw
						.comment("Define a base url for the geoserver application.\n"
			...
			if ((g.getAdminUserName() != null) && (g.getAdminUserName() != "")) {
				cw
						.comment("Defines the user name of the administrator for log in\n"
			...
			if ((g.getAdminPassword() != null) && (g.getAdminPassword() != "")) {
				cw
						.comment("Defines the password of the administrator for log in\n"
								+ "to the web based administration tool.");
			...
			if ((g.getTileCache() != null)
					&& !"".equals(g.getTileCache().trim())) {
				cw
						.comment("Defines hte location of a tile cache (full url or relative path)");
	...
	protected static void storeService(Object obj, WriterHelper cw)
		...
		if (obj instanceof WCSDTO) {
			...
			throw new ConfigurationException(
					"Invalid object: not WMS or WFS or WCS");
		}
		...
		cw
				.comment("ServiceDTO elements, needed for the capabilities document\n"
		...
		if ((baseMapLayers != null) && (baseMapStyles != null)
				&& (baseMapEnvelopes != null)) {
			cw.openTag("BaseMapGroups");

			// for each title/layer combo, write it out
			String[] titles = (String[]) baseMapLayers.keySet().toArray(
					new String[0]);
			...
			for (int i = 0; i < titles.length; i++) {
				...
				cw.textTag("baseMapLayers", (String) baseMapLayers
						.get(titles[i]));
				cw.textTag("baseMapStyles", (String) baseMapStyles
						.get(titles[i]));

				GeneralEnvelope e = (GeneralEnvelope) baseMapEnvelopes
						.get(titles[i]);
				Map m = new HashMap();

				m.put("srsName", e.getCoordinateReferenceSystem()
	...
	protected static void storeCatalog(WriterHelper cw, DataDTO data)
		...
		cw
				.comment("a datastore configuration element serves as a common data source connection\n"
		...
		cw.openTag("formats");
		cw
				.comment("a format configuration element serves as a common data source\n"
		...
		cw.openTag("styles");
		cw
				.comment("Defines the style ids and file name to be used by the wms.");
	...
	protected static void storeStyle(WriterHelper cw, StyleDTO s)
			throws ConfigurationException {
		if (LOGGER.isLoggable(Level.FINER)) {
			LOGGER.finer(new StringBuffer("In method storeStyle: ").append(s)
					.toString());
		}
		...
		if (LOGGER.isLoggable(Level.FINER)) {
			LOGGER.finer(new StringBuffer("storing style ").append(attr)
					.toString());
		}
	...
	protected static void storeFeatures(File dir, DataDTO data)
		...
		while (i.hasNext()) {
			String s = (String) i.next();
			FeatureTypeInfoDTO ft = (FeatureTypeInfoDTO) data
					.getFeaturesTypes().get(s);
			...
			if (ft != null) {
				...
				try { // encode the file name (this is to catch colons in FT
						// names)
					ftDirName = URLEncoder.encode(ftDirName,
							getDefaultEncoding());

					if (LOGGER.isLoggable(Level.FINER)) {
						LOGGER.finer(new StringBuffer("Writing encoded URL: ")
								.append(ftDirName).toString());
					}
				} catch (UnsupportedEncodingException e1) {
					throw new ConfigurationException(e1);
				}

				File dir2 = WriterUtils.initWriteFile(new File(dir, ftDirName),
						true);
				...
				if (ft.getSchemaAttributes() != null) {
					if (LOGGER.isLoggable(Level.FINER)) {
						LOGGER.finer(new StringBuffer(ft.getKey()).append(
								" writing schema.xml w/ ").append(
		...
		for (int j = 0; j < fa.length; j++) {
			...
			while ((fti == null) && i.hasNext()) {
				...
				try { // encode the file name (this is to catch colons in FT
					...
					if (LOGGER.isLoggable(Level.FINER)) {
						LOGGER.finer(new StringBuffer("Decoded URL: ").append(
								ftDirName).toString());
					}
				} catch (UnsupportedEncodingException e1) {
					throw new ConfigurationException(e1);
	...
	protected static void storeFeature(FeatureTypeInfoDTO ft, File dir)
		...
		try {
			...
			if ((ft.getMetadataLinks() != null)
					&& (ft.getMetadataLinks().size() != 0)) {
				cw.openTag("metadataLinks");

				for (Iterator it = ft.getMetadataLinks().iterator(); it
						.hasNext();) {
			...
			if ((ft.getDefaultStyle() != null) && (ft.getDefaultStyle() != "")) {
				cw
						.comment("the default style this FeatureTypeInfoDTO can be represented by.\n"
			...
			if (ft.getDefinitionQuery() != null) {
				...
				String sfilter = ftransformer
						.transform(ft.getDefinitionQuery());
				cw.writeln(sfilter);
			}
	...
	protected static void storeFeatureSchema(FeatureTypeInfoDTO fs, File dir)
		...
		if ((fs.getSchemaBase() == null) || (fs.getSchemaBase() == "")) {
			...
			if (LOGGER.isLoggable(Level.FINER)) {
				LOGGER.finer(new StringBuffer(fs.getKey()).append(
						" has not schemaBase").toString());
			}
	...
	public static void storeFeatureSchema(FeatureTypeInfoDTO fs, Writer w)
		...
		for (int i = 0; i < fs.getSchemaAttributes().size(); i++) {
			AttributeTypeInfoDTO ati = (AttributeTypeInfoDTO) fs
					.getSchemaAttributes().get(i);
			m = new HashMap();
			m.put("nillable", "" + ati.isNillable());
			m.put("minOccurs", "" + ati.getMinOccurs());
			m.put("maxOccurs", "" + ati.getMaxOccurs());

			NameSpaceTranslator nst_xs = NameSpaceTranslatorFactory
					.getInstance().getNameSpaceTranslator("xs");
			NameSpaceTranslator nst_gml = NameSpaceTranslatorFactory
	...
	protected static void storeCoverages(File dir, DataDTO data)
		...
		while (i.hasNext()) {
			...
			if (cv != null) {
				File dir2 = WriterUtils.initWriteFile(new File(dir, cv
						.getDirName()), true);
	...
	protected static void storeCoverage(CoverageInfoDTO cv, File dir)
		...
		try {
			...
			// //
			if ((cv.getVerticalExtent() != null)
					&& !cv.getVerticalExtent().isEmpty()) {
			...
			// //
			if ((cv.getTemporalExtent() != null)
					&& !cv.getTemporalExtent().isEmpty()) {
				if (cv.getTemporalExtent().containsKey("timePositions")) {
					final List values = (List) cv.getTemporalExtent().get(
							"timePositions");
					...
					final Interval period = (Interval) cv.getTemporalExtent()
							.get("timePeriod");

					if (period != null) {
						cw.openTag("temporalExtent");
						cw.textTag("beginTime", String.valueOf(period
								.getStartMillis()));
						cw.textTag("endTime", String.valueOf(period
								.getEndMillis()));
						cw.closeTag("temporalExtent");
					}
				}
			...
			if (cv.getGrid() != null) {
				...
				m
						.put("dimension", new Integer(g.getGridRange()
								.getDimension()));
				...
				if (tx instanceof AffineTransform) {
					...
					cw.textTag("translateX", String
							.valueOf(aTX.getTranslateX()));
					cw.textTag("translateY", String
							.valueOf(aTX.getTranslateY()));
			...
			if (cv.getDimensions() != null) {
				...
				for (int d = 0; d < dims.length; d++) {
					...
					if (dims[d].getRange() != null) {
						cw.openTag("interval");
						cw.textTag("min", Double.toString(dims[d].getRange()
								.getMinimum(true)));
						cw.textTag("max", Double.toString(dims[d].getRange()
								.getMaximum(true)));
			...
			if ((cv.getSupportedFormats() != null)
					&& (cv.getSupportedFormats().size() != 0)) {
				String s = "";
			...
			//
			// ///////////////////////////////////////////////////////////////////////
			if ((cv.getParameters() != null)
					&& (cv.getParameters().size() != 0)) {
				...
				while (i.hasNext()) {
					...
					if ("values_palette".equalsIgnoreCase(key)) {
						...
						if (cv.getParameters().get(key) != null) {
							temp.put("name", key);
							temp.put("value", cv.getParameters().get(key)
									.toString().replaceAll("\"", "'"));
						}
					}
	...
	public static class WriterUtils {
		...
		public static File initFile(File f, boolean isDir)
			...
			if (!f.exists()) {
				if (LOGGER.isLoggable(Level.FINER)) {
					LOGGER.finer(new StringBuffer("Creating File: ").append(
							f.toString()).toString());
				}
					...
					try {
						if (LOGGER.isLoggable(Level.SEVERE)) {
							LOGGER.severe(new StringBuffer(
									"Attempting to create file:").append(
			...
			if (LOGGER.isLoggable(Level.FINER)) {
				LOGGER.finer(new StringBuffer("File is valid: ").append(f)
						.toString());
			}
