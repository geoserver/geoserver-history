...
public class TileLevelTest extends RegionatingTestSupport {
    ...
    public void testDeepTiles() throws Exception {
        ...
        TileLevel root = TileLevel.makeRootLevel(CachedHierarchyRegionatingStrategy.getWorldBounds(), 1, dummyComp);
    ...
    public void testFindTile() throws Exception {
        ...
        assertEquals(0, getZoomLevel(root.findTile(boxes[0]))); 
        assertEquals(1, getZoomLevel(root.findTile(boxes[1])));
        assertEquals(1, getZoomLevel(root.findTile(boxes[2])));
    }

    private long getZoomLevel(TileLevel t){
        long z = 
            CachedHierarchyRegionatingStrategy.getTileCoords(
                    t.getBounds(),
