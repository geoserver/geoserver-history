...
public class GetMapResponse implements Response {
	...
	public void execute(Request req) throws ServiceException {
		...
		delegate = getDelegate(outputFormat, wms);
			...
			for (int i = 0; i < length; i++) {
				...
				if (layers[i].getType() == MapLayerInfo.TYPE_VECTOR) {
					...
					if (reader != null) {
						...
						//
						// A workaround for subsetting coverages configured from
						// a netcdf file
						...
						// parsing
						if (reader.getFormat().getName().equalsIgnoreCase(
								"netcdf")) {
							...
							if (request.getElevation() != null) {
								...
								ParameterDescriptor elevDescriptor = (ParameterDescriptor) elevation
										.getDescriptor();
								elevation.setValue(elevDescriptor
										.getDefaultValue());
							}
						}
						...
						layer = new DefaultMapLayer(
								DataUtilities.wrapGcReader(reader,
	...
	private GetMapProducer getDelegate(String outputFormat, WMS wms)
		...
		final WmsException e = new WmsException(
	 ...
	 * @return DOCUMENT ME!
	 */
	public synchronized static Set loadImageFormats(ApplicationContext applicationContext) {
		final Map beans = applicationContext
				.getBeansOfType(GetMapProducerFactorySpi.class);
		final Collection producers = beans.values();
