 ...
 * @version $Id: GetLegendGraphicKvpReader.java 7959 2007-12-06 16:52:34Z
 *          afabiani $
...
public class GetLegendGraphicKvpReader extends WmsKvpRequestReader {
	...
	public Request getRequest(HttpServletRequest httpRequest)
		...
		try {
			...
			mli.setFeature(fti);
			request.setLayer(mli);
		} catch (NoSuchElementException e) {
			try {
				CoverageInfo cvi = request.getWMS().getData().getCoverageInfo(
						layer);
				mli.setCoverage(cvi);
				request.setLayer(mli);
			} catch (NoSuchElementException ne) {
				throw new WmsException(ne, new StringBuffer(layer).append(
						" layer does not exists.").toString(), ne
						.getLocalizedMessage());
			} catch (FactoryConfigurationError fce) {
				throw new WmsException(fce,
						"Can't obtain the schema for the required layer.", fce
								.getLocalizedMessage());
			}
		}
		...
		if (getServiceRef().getApplicationContext() == null) {
			...
			throw new WmsException(new StringBuffer("Invalid graphic format: ")
					.append(format).toString(), "InvalidFormat");
		} else {
	...
	private void parseOptionalParameters(GetLegendGraphicRequest req,
		...
		req.setLegendOptions(Requests
				.parseOptionParameter(getValue("LEGEND_OPTIONS")));
	}
	...
	private void parseStyleAndRule(GetLegendGraphicRequest req,
		...
		if (LOGGER.isLoggable(Level.FINE)) {
			LOGGER.fine(new StringBuffer("looking for style ")
					.append(styleName).toString());
		}
	...
	private Style findStyle(String styleName, Style[] styles)
		...
		if (styleName == null) {
			if (LOGGER.isLoggable(Level.FINER)) {
				LOGGER
						.finer("styleName is null, request in literal mode, returning first style");
			}
