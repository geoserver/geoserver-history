...
public class GetMapRequest extends WMSRequest {
	...
	/**
	 * DJB: spec says SRS is *required*, so if they dont specify one, we should
	 * throw an error instead we use "NONE" - which is no-projection. Previous
	 * behavior was to the WSG84 lat/long (4326)
	 * 
	 * @return request CRS, or <code>null</code> if not set. TODO: make CRS
	 *         manditory as for spec conformance
	 */
	...
	/**
	 * Gets a list of the the filters that will be applied to each layer before
	 * rendering
	 * 
	 * @return -
	 */
	...
	/**
	 * @return super overlay flag, <code>true</code> if super overlay
	 *         requested.
	 */
	...
	/**
	 * @param score
	 *            the KML/KMZ score value for image vs. vector response, from 0
	 *            to 100
	 */
	...
	private class OptionalParameters {
		...
		/**
		 * vendor extensions, allows to filter each layer with a user defined
		 * filter
		 */
		List filters;
		...
		/**
		 * Temporary hack since finding a good tiling origin would require us to
		 * compute the bbox on the fly TODO: remove this once we cache the real
