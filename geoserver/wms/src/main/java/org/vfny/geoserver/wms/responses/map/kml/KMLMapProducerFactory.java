...
/**
 * KMLMapProducerFactory This class is used as part of the SPI auto discovery
...
public class KMLMapProducerFactory implements GetMapProducerFactorySpi {
     ...
     *
     * <p>
     * In this case, true if <code>mapFormat</code> starts with "kml", as both
     * <code>"kml"</code> and <code>"kml+xml"</code> are commonly passed.
     * Also true if <code>application/vnd.google-earth.kml</code> is passed,
     * since that is as close as we can get to the value advertised in the
     * capabilities document taking into account the common mistake of not
     * escaping '+'. We must do this as the most way to request an output format
     * is to use the name in the caps document.
     * </p>
     *
