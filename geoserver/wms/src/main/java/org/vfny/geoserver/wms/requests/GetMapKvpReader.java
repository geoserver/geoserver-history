...
public class GetMapKvpReader extends WmsKvpRequestReader {
	...
	public void parseOptionalParameters(GetMapRequest request)
		...
		if (paletteValue != null) {
			try {
				...
				request.setPaletteInverter(eicm);
			} catch (Exception e) {
	...
	private Point2D parseTilesOrigin(String origin) throws WmsException {
		...
		if (coordValues.length != 2) {
			throw new WmsException(origin + " is not a valid coordinate",
					getClass().getName());
		}
			...
			if (layerType == null) {
				...
				for (Iterator c_keys = catalog.getLayerNames().iterator(); c_keys
					...
					if (counter == l_counter) {
						...
						throw new WmsException(new StringBuffer(layerName)
								.append(": no such layer on this server")
								.toString(), "LayerNotDefined");
					}
				}
			} else {
