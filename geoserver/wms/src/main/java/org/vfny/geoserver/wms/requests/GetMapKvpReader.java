 ...
 * 
 * <ul>
 * <li> LAYERS layer names, as exposed by the capabilities document, to compose a map with, in the
 * order they may appear, being the first layer the one at the bottom of the layer stack and the
 * last one the one at the top. </li>
 * <li> STYLES list of named styles known by this server and applicable to the requested layers. It
 * can be empty or contain exactly as many style names as layers was requested, in which case empty
 * strings could be used to denote that the default layer style should be used. (exaple:
 * <code>LAYERS=buildings,roads,railroads&STYLES=,centerline,</code>. This example says create a
 * map with roads layer using its default style, roads with "centerline" style, and railroads with
 * its default style. </li>
 * <li> BBOX Area of interest for which to contruct the map image, in the Coordinate Reference
 * System given by the SRS parameter. </li>
 * <li> FORMAT MIME type of the resulting map, must be one of the advertised in the capabilities
 * document. </li>
 * <li> WIDTH desired map witdth in output units (pixels). UNITS support should be added to the
 * spec, and UNITS and DPI parameters added. </li>
 * <li> HEIGHT desired map height in output units (pixels). UNITS support should be added to the
 * spec, and UNITS and DPI parameters added. </li>
 * </ul>
 * </p>
 * 
 ...
 * <ul>
 * <li> SRS </li>
 * <li> TRANSPARENT boolean indicatin wether to create a map with transparent background or not (if
 * transparency is supported by the requested output format). </li>
 * <li> EXCEPTIONS MIME type of the exception report. </li>
 * <li> BGCOLOR map background color, in <code>0xRRGGBB</code> format. </li>
 * <li> SLD client supplies a URL for a remote SLD document through this parameter. This parameter
 * takes precedence over STYLES. If present, replaces the LAYERS and STYLES parameters, since
 * they're defined in the remote document itself. The document send by this way will be used in
 * "literal" or "library" mode, see explanation bellow. </li>
 * <li> SLD_BODY client spplies the SLD document itself through this parameter, properly encoded for
 * an HTTP query string. This parameter takes precendence over STYLES and SLD. If present, replaces
 * the LAYERS and STYLES parameters, since they're defined in the inline document itself. The
 * document send by this way will be used in "literal" or "library" mode, see explanation bellow.
 * </li>
 * </ul>
 * </p>
 * 
 * <p>
 * As defined by the Styled Layer Descriptor specification, version 1.0.0, the SLD document supplied
 * by the SLD or SLD_BODY parameter can be used in "literal" or "library" mode, depending on whether
 * the <strong>LAYERS=</strong> parameter is present.
 * </p>
 * 
 * <p>
 * Here is the explanation from the spec, section 6.4, page 10: "the SLD can also be used in one of
 * two different modes depending on whether the LAYERS parameter is present in the request. If it is
 * not present, then all layers identified in the SLD document are rendered with all defined styles,
 * which is equivalent to the XML-POST method of usage. If the LAYERS parameter is present, then
 * only the layers identified by that parameter are rendered and the SLD is used as a style library . "
 * </p>
 * 
...
public class GetMapKvpReader extends WmsKvpRequestReader {
    ...
    /**
     * Indicates wether STYLES parameter must be parsed. Defaults to <code>true</code>, but can
     * be set to false, for example, when parsing a GetFeatureInfo request, which shares most of the
     * getmap parameter but not STYLES.
     * 
    ...
    /**
     * Indicates whether the FORMAT parameter is required or not. Defaults to true as per the GetMap
     * request requires in the spec, but can be set to false for a GetFeatureInfo request using this
     * parser to be permisive about the FORMAT parameter.
     */
    ...
    /**
     * Produces a <code>GetMapRequest</code> instance by parsing the GetMap mandatory, optional
     * and custom parameters.
     * 
     * @param httpRequest
     *                the servlet request who's application object holds the server configuration
     * 
     * @return a <code>GetMapRequest</code> completely setted up upon the parameters passed to
     *         this reader
     * 
    ...
    public void parseOptionalParameters(GetMapRequest request) throws WmsException {
        ...
        if (KMScore != null) {
            try {
                ...
                throw new WmsException("KMScore parameter (" + KMScore
     ...
     * @param request
     *                DOCUMENT ME!
     * @parseStylesLayers true = normal operation, false = dont parse the styles and layers (used by
     *                    the SLD GET/POST)
     * 
     * @throws WmsException
     *                 DOCUMENT ME!
    ...
    /**
     * creates a list of requested attributes, wich must be a valid attribute name or one of the
     * following special attributes:
     * 
     * <ul>
     * <li> <b>#FID</b>: a map producer capable of handling attributes (such as SVGMapResponse),
     * will write the feature id of each feature </li>
     * <li> <b>#BOUNDS</b>: a map producer capable of handling attributes (such as SVGMapResponse),
     * will write the bounding box of each feature </li>
     * </ul>
     * 
     * 
     * @param layers
     ...
     *         <code>List&lt;List&lt;String&gt;&gt;</code> with an entry for each requested layer,
     *         where each of them consists of a List of the attribute names requested
     * 
     * @throws WmsException
     *                 if:
     *                 <ul>
     *                 <li>the number of attribute sets requested is not equal to the number of
     *                 layers requested.</li>
     *                 <li>an illegal attribute name was requested</li>
     *                 <li>an IOException occurs while fetching a FeatureType schema to ask it for
     *                 propper attribute names</li>
     *                 </ul>
     */
    private List parseAttributes(FeatureTypeInfo[] layers) throws WmsException {
        ...
        if (byFeatureTypes.size() != nLayers) {
            throw new WmsException(byFeatureTypes.size()
                    + " lists of attributes specified, expected " + layers.length, getClass()
                    .getName()
                    + "::parseAttributes()");
        }
        ...
        for (int i = 0; i < nLayers; i++) {
            ...
            try {
                ...
                for (Iterator attIt = atts.iterator(); attIt.hasNext();) {
                    ...
                    if (attName.length() > 0) {
                        if (LOGGER.isLoggable(Level.FINER)) {
                            LOGGER.finer(new StringBuffer("checking that ").append(attName).append(
                                    " is valid").toString());
                        }

                        if ("#FID".equalsIgnoreCase(attName) || "#BOUNDS".equalsIgnoreCase(attName)) {
                            if (LOGGER.isLoggable(Level.FINER)) {
                                LOGGER.finer(new StringBuffer("special attribute name requested: ")
    ...
    /**
     * Parses the list of style names requested for each requested layer and looks up the actual
     * Style objects, which are returned in an ordered list.
     * 
     * <p>
     * A client _may_ request teh default Style using a null value (as in "STYLES="). If several
     * layers are requested with a mixture of named and default styles, the STYLES parameter
     * includes null values between commas (as in "STYLES=style1,,style2,,"). If all layers are to
     * be shown using the default style, either the form "STYLES=" or "STYLES=,,," is valid.
     * </p>
     * 
     * @param request
     *                DOCUMENT ME!
     ...
     * 
     * @return a full <code>List</code> of the style names requested for the requiered layers with
     *         no null style names.
     * 
     * @throws WmsException
     *                 if some of the requested styles does not exist or its number if greater than
     *                 zero and distinct of the number of requested layers
     */
            ...
            for (int i = 0; i < numLayers; i++) {
                ...
                if (currLayer.getType() == MapLayerInfo.TYPE_VECTOR) {
                    ...
                    if (LOGGER.isLoggable(Level.FINE)) {
                        LOGGER.fine(new StringBuffer("establishing ").append(currStyleName).append(
                                " style for ").append(layers[i].getName()).toString());
                    }
     ...
     * @param currStyleName
     * 
     * @return the configured style named <code>currStyleName</code> or <code>null</code> if
     *         such a style does not exists on this server.
     */
     ...
     * <code>StyledLayerDescriptor</code>, then takes the layers and styles to use in the map
    ...
    protected void parseSldBodyParam(GetMapRequest request) throws WmsException {
        ...
        if (getValue("VALIDATESCHEMA") != null) {
            ...
            errors = validator.validateSLD(in, request.getHttpServletRequest().getSession()
                    .getServletContext());
    ...
    /**
     * Create a reader of the given String. This reader will be used in the InputSource for the sld
     * parser. The advantage with a reader over a input stream is that we don't have to consider
     * encoding. The xml declaration with encoding is ignored using a Reader in parser. The encoding
     * of the string has been appropiate handled by the servlet when streaming in.
     * 
     * @param sldBody
    ...
    /**
     * Gets a sequence of url encoded filters and parses them into Filter objects that will be set
    ...
    protected void parseSldParam(GetMapRequest request) throws WmsException {
        ...
        if (getValue("VALIDATESCHEMA") != null) {
            ...
            try {
                ...
                if (errors.size() != 0) {
                    throw new WmsException(SLDValidator
                            .getErrorMessage(sldUrl.openStream(), errors));
                }
            } catch (IOException e) {
                String msg = new StringBuffer("Creating remote SLD url: ").append(e.getMessage())
                        .toString();
     ...
     * 
     * <p>
     * If <code>sld</code> is used in "library" mode, that is, the LAYERS param is also present,
     * saying what layers must be taken in count, then only the layers from the LAYERS parameter are
     * used and <code>sld</code> is used as a style library, which means that for each layer
     * requested through LAYERS=..., if a style if found in it for that layer it is used, and if
     * not, the layers default is used.
     * </p>
     * 
     * <p>
     * By the other hand, if the LAYERS parameter is not present all the layers found in
     * <code>sld</code> are setted to <code>request</code>.
     * </p>
     * 
     * @param request
            ...
            for (int i = 0; i < slCount; i++) {
                ...
                if ((sl instanceof UserLayer)
                    ...
                    try {
                        ...
                        currLayer.setCoverage(
                                GetMapKvpReader.findCoverageLayer(request, layerName), null);
                    }
                }
        ...
        if (ftcs != null) {
            ...
            for (int t = 0; t < length; t++) {
                ...
                if (ftc.getFeatureTypeName() != null) {
                    ...
                    try {
                        final SimpleFeatureType currSchema = currLayer.getFeature()
                                .getFeatureType();
    ...
    /**
     * Finds the style for <code>layer</code> in <code>styledLayers</code> or the layer's
     * default style if <code>styledLayers</code> has no a UserLayer or a NamedLayer with the same
     * name than <code>layer</code>
     * <p>
     ...
     * 
     * @param layer
     *                one of the internal FeatureType that was requested through the LAYERS
     *                parameter or through and SLD document when the request is in literal mode.
     * @param styledLayers
    ...
    /**
     * Parses the requested layers given by the LAYERS request parameter and looks up their
     * corresponding FeatureTypeInfo objects in the server.
     * 
     * @param request
     * 
     * @return
