...
/**
 * Template {@linkPlain
 * org.vfny.geoserver.responses.wms.GetLegendGraphicProducer} based on GeoTools'
 * {@link
 * http://svn.geotools.org/geotools/trunk/gt/module/main/src/org/geotools/renderer/lite/StyledShapePainter.java
 * StyledShapePainter} that produces a BufferedImage with the appropiate legend
 * graphic for a given GetLegendGraphic WMS request.
 * 
 * <p>
 ...
 * representative enough of the SLD style for which it is being generated. Thus,
 * if no <code>RULE</code> parameter was passed and the style has more than
 * one applicable Rule for the actual scale factor, there will be generated a
 * legend graphic of the specified width, but with as many stacked graphics as
 * applicable rules were found, providing by this way a representative enough
 * legend.
 * </p>
 * 
 * @author Gabriel Roldan, Axios Engineering
 * @version $Id: DefaultRasterLegendProducer.java 7973 2007-12-12 16:05:02Z
 *          afabiani $
 */
public abstract class DefaultRasterLegendProducer implements
		GetLegendGraphicProducer {
	/** shared package's logger */
	private static final Logger LOGGER = Logger
			.getLogger(DefaultRasterLegendProducer.class.getPackage().getName());
	...
	/**
	 * Singleton shape painter to serve all legend requests. We can use a single
	 * shape painter instance as long as it remains thread safe.
	 */
	 ...
	 * indicates that the rendering of the legend graphic should stop gracefully
	 * as soon as possible
	 */
	 ...
	 * 
	 * @param request
	 *            the "parsed" request, where "parsed" means that it's values
	 *            are already validated so this method must not take care of
	 *            verifying the requested layer exists and the like.
	 * 
	 * @throws WmsException
	 *             if there are problems creating a "sample" feature instance
	 *             for the FeatureType <code>request</code> returns as the
	...
	public void produceLegendGraphic(GetLegendGraphicRequest request)
		...
		try {
			if (request.getLayer().getType() == MapLayerInfo.TYPE_VECTOR)
				sampleFeature = createSampleFeature(request.getLayer()
						.getFeature().getFeatureType());
			else {
				FeatureCollection feature = FeatureUtilities
						.wrapGridCoverage((GridCoverage2D) request.getLayer()
								.getCoverage().getCoverage(null, null));
				sampleFeature = createSampleFeature(feature.getFeatureType());
			}
		} catch (IOException e) {
		...
		for (int i = 0; i < ruleCount; i++) {
			...
			for (int sIdx = 0; sIdx < symbolizers.length; sIdx++) {
				...
				if (symbolizer instanceof RasterSymbolizer) {
					...
					try {
						if (request.getLayer().getCoverage() != null
								&& request.getLayer().getCoverage()
										.isAllowWatermarking()
								&& request.getLayer().getCoverage()
										.getWatermarkingURL() != null) {

							String wmURL = request.getLayer().getCoverage()
									.getWatermarkingURL();

							if (!(wmURL.toLowerCase().startsWith("http://") || wmURL
									.toLowerCase().startsWith("file:/")))
								wmURL = request.getHttpServletRequest()
										.getRequestURL()
										+ "/" + wmURL;
							imgShape = ImageIO.read(new URL(wmURL));
						} else {
							imgShape = ImageIO.read(new URL(request
					...
					image = new BufferedImage(imgShape.getWidth(), imgShape
							.getHeight(), BufferedImage.TYPE_INT_ARGB);
					graphics = image.createGraphics();
					graphics.setRenderingHint(RenderingHints.KEY_ANTIALIASING,
							RenderingHints.VALUE_ANTIALIAS_ON);
					graphics.setColor(bgColor);
					graphics.fillRect(0, 0, imgShape.getWidth(), imgShape
							.getHeight());
					graphics.drawImage(imgShape, 0, 0, imgShape.getWidth(),
							imgShape.getHeight(), null);
				} else {
	 ...
	 *       handleLegendURL() in WMSCapsTransformer.
	 * @param image
	 * @return
	 */
	private BufferedImage scaleImage(BufferedImage image,
		...
		xform.setToScale(((double) w) / image.getWidth(), ((double) h)
				/ image.getHeight());
	...
	private static BufferedImage mergeLegends(List imageStack, Rule[] rules,
		...
		if (req.getLegendOptions().get("fontAntiAliasing") instanceof String) {
			String aaVal = (String) req.getLegendOptions().get(
					"fontAntiAliasing");
		...
		if (req.getLegendOptions().get("forceLabels") instanceof String) {
			String forceLabelsOpt = (String) req.getLegendOptions().get(
					"forceLabels");
		...
		if (imageStack.size() == 1 && !forceLabelsOn) {
			...
			for (int i = 0; i < imgCount; i++) {
				...
				if (forceLabelsOff) {
					totalWidth = (int) Math.ceil(Math.max(img.getWidth(),
							totalWidth));
					...
					if (labels[i] != null && labels[i].length() > 0) {
						final BufferedImage renderedLabel = renderLabel(
								labels[i], g, req);
						final Rectangle2D bounds = new Rectangle2D.Double(0, 0,
								renderedLabel.getWidth(), renderedLabel
										.getHeight());

						totalWidth = (int) Math.ceil(Math.max(img.getWidth()
								+ bounds.getWidth(), totalWidth));
						rowHeights[i] = (int) Math.ceil(Math.max(img
			...
			for (int i = 0; i < imgCount; i++) {
				...
				if (labels[i] != null && labels[i].length() > 0) {
					...
					final BufferedImage renderedLabel = renderLabel(labels[i],
							finalGraphics, req);
					...
					finalGraphics.drawImage(renderedLabel, img.getWidth(), y,
							imgObs);
				}
	 ...
	 * '\n' '\r' and '\f' are interpreted as linebreaks, as is the characater
	 * combination "\n" (as opposed to the actual '\n' character). This allows
	 * people to force line breaks in their labels by including the character
	...
	public static BufferedImage renderLabel(String label, Graphics2D g,
		...
		// to indicate a line break, as well as a traditional 'real' line-break
		// in the XML.
		BufferedImage renderedLabel;
		Color labelColor = getLabelFontColor(req);
		if ((label.indexOf("\n") != -1) || (label.indexOf("\\n") != -1)) {
			// this is a label WITH line-breaks...we need to figure out it's
			// height *and*
			...
			while (st.hasMoreElements()) {
				...
				Rectangle2D thisLineBounds = g.getFontMetrics()
						.getStringBounds(token, g);

				// if this is directly added as thisLineBounds.getHeight(), then
				// there are rounding errors
				// because we can only DRAW fonts at discrete integer coords.
				final int thisLineHeight = (int) Math.ceil(thisLineBounds
						.getHeight());
				bounds.add(0, thisLineHeight + bounds.getHeight());
				bounds.add(thisLineBounds.getWidth(), 0);
				lineHeight.add(new Integer((int) Math.ceil(thisLineBounds
						.getHeight())));
			}

			// make the actual label image
			renderedLabel = new BufferedImage((int) Math
					.ceil(bounds.getWidth()), (int) Math.ceil(bounds
					.getHeight()), BufferedImage.TYPE_INT_ARGB);
			...
			rlg.setRenderingHint(RenderingHints.KEY_TEXT_ANTIALIASING, g
			...
			int height = (int) Math.ceil(g.getFontMetrics().getStringBounds(
					label, g).getHeight());
			int width = (int) Math.ceil(g.getFontMetrics().getStringBounds(
					label, g).getWidth());
			renderedLabel = new BufferedImage(width, height,
					BufferedImage.TYPE_INT_ARGB);
			...
			rlg.setRenderingHint(RenderingHints.KEY_TEXT_ANTIALIASING, g
					.getRenderingHint(RenderingHints.KEY_TEXT_ANTIALIASING));
			rlg
					.drawString(label, 0, height
							- rlg.getFontMetrics().getDescent());
		}
		...
		for (int i = 0; i < ftStyles.length; i++) {
			...
			for (int j = 0; j < rules.length; j++) {
				...
				if (isWithInScale(r, scaleDenominator)) {
					...
					/*
					 * I'm commented this out since I guess it has no sense for
					 * producing the legend, since wether or not the rule has an
					 * else filter, the legend is drawn only if the scale
					 * denominator lies inside the rule's scale range. if
					 * (r.hasElseFilter()) { ruleList.add(r); }
					 */
				}
			}
		}
	 ...
	 *            The rule
	 * @param scaleDenominator
	 *            the scale denominator to check if it is between the rule's
	 *            scale range. -1 means that it allways is.
	 * 
	 * @return true if the scale is compatible with the rule settings
	 */
	private boolean isWithInScale(Rule r, double scaleDenominator) {
		return (scaleDenominator == -1)
				|| (((r.getMinScaleDenominator() - TOLERANCE) <= scaleDenominator) && ((r
						.getMaxScaleDenominator() + TOLERANCE) > scaleDenominator));
	}
