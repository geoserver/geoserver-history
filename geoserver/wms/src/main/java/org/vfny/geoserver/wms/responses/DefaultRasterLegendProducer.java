...
public abstract class DefaultRasterLegendProducer implements GetLegendGraphicProducer {
                ...
                if (symbolizer instanceof RasterSymbolizer) {
                    ...
                    try {
                        if ((request.getLayer().getCoverage() != null)
                                && request.getLayer().getCoverage().isAllowWatermarking()
                                && (request.getLayer().getCoverage().getWatermarkingURL() != null)) {
                            String wmURL = request.getLayer().getCoverage().getWatermarkingURL();

                            if (!(wmURL.toLowerCase().startsWith("http://")
                                    || wmURL.toLowerCase().startsWith("file:/"))) {
                                wmURL = request.getHttpServletRequest().getRequestURL() + "/"
                                    + wmURL;
                            }
                    ...
                    image = new BufferedImage(imgShape.getWidth(), imgShape.getHeight(),
                    ...
                    graphics.drawImage(imgShape, 0, 0, imgShape.getWidth(), imgShape.getHeight(),
                        null);
                } else {
    ...
    private static Font getLabelFont(GetLegendGraphicRequest req) {
        ...
        if (legendOptions.get("fontSize") != null) {
            try {
                ...
                LOGGER.warning("Error trying to interpret legendOption 'fontSize': "
    ...
    private static Color getLabelFontColor(GetLegendGraphicRequest req) {
        ...
        try {
            ...
            LOGGER.warning("Could not decode label color: " + color + ", default to "
                + FONT_COLOR.toString());
    ...
    private static Color getBackgroundColor(GetLegendGraphicRequest req) {
        ...
        try {
            ...
            LOGGER.warning("Could not decode background color: " + color + ", default to "
                + BG_COLOR.toString());
