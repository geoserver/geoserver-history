...
/**
 * Abstract class to do the common work of the FeatureInfoResponse
 * subclasses. Subclasses should just need to implement writeTo(), to write
 * the actual response, the executions are handled here, figuring out where on
 * the map the pixel is located.<p>Would be nice to have some greater
 * control over the pixels that are selected. Ideally we would be able to
 * detect things like the size of the mark, so that users need not click on
 * the exact center, or the exact pixel. This is not a big deal for polygons,
 * but is for lines and points.  One half solution to make things a bit nicer
 * would be a global parameter to set a wider pixel range.</p>
 *
...
public abstract class AbstractFeatureInfoResponse extends GetFeatureInfoDelegate {
    ...
    /**
     * setted in execute() from the requested output format, it's
    ...
    /**
     * Returns the content encoding for the output data.<p>Note that
     * this reffers to an encoding applied to the response stream (such as
     * GZIP or DEFLATE), and not to the MIME response type, wich is returned
     * by <code>getContentType()</code></p>
     *
     ...
     * world coordinates.  Stolen from LiteRenderer but without the need of a
     * Graphics object
     *
