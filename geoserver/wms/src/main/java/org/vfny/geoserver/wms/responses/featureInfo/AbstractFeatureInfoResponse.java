    ...
    private FeatureCollection wrapPixelInFeatureCollection(
        ...
        try {
            for (int i = 0; i < types.length; i++) {
                types[i] = AttributeTypeFactory.newAttributeType(sampleDimensions[i].getDescription().toString(), Double.class);
            }
            gridType = FeatureTypeBuilder.newFeatureType(types, coverageName);
        } catch(Exception e) {
            // sometimes a grid coverage format does not assign unique descriptions to coverages
            for (int i = 0; i < types.length; i++) {
                types[i] = AttributeTypeFactory.newAttributeType("Band " + (i + 1), Double.class);
            }
            gridType = FeatureTypeBuilder.newFeatureType(types, coverageName);
        }
        ...
        return DataUtilities.collection(gridType.create(values, ""));
    }
