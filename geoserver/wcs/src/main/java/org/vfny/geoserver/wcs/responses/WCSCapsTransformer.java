...
public class WCSCapsTransformer extends TransformerBase {
	...
	private static class WCSCapsTranslator extends TranslatorSupport {
		...
		private void handleEnvelope(GeneralEnvelope lonLatEnvelope,
				Map verticalExtent, Map temporalExtent) {
			AttributesImpl attributes = new AttributesImpl();
			...
			if (verticalExtent != null && !verticalExtent.isEmpty()) {
				...
				try {
					firstPosition = Double
							.parseDouble((String) ((LinkedList) values)
									.getFirst());
				...
				element("gml:pos", new StringBuffer(Double
						.toString(lonLatEnvelope.getLowerCorner()
								.getOrdinate(0))).append(" ").append(
						lonLatEnvelope.getLowerCorner().getOrdinate(1)).append(
						" ").append(firstPosition).toString());
				element("gml:pos", new StringBuffer(Double
						.toString(lonLatEnvelope.getUpperCorner()
								.getOrdinate(0))).append(" ").append(
						lonLatEnvelope.getUpperCorner().getOrdinate(1)).append(
						" ").append(lastPosition).toString());
			} else {
				element("gml:pos", new StringBuffer(Double
						.toString(lonLatEnvelope.getLowerCorner()
								.getOrdinate(0))).append(" ").append(
						lonLatEnvelope.getLowerCorner().getOrdinate(1))
						.toString());
				element("gml:pos", new StringBuffer(Double
						.toString(lonLatEnvelope.getUpperCorner()
								.getOrdinate(0))).append(" ").append(
						lonLatEnvelope.getUpperCorner().getOrdinate(1))
						.toString());
			}
			...
			if (temporalExtent != null && !temporalExtent.isEmpty()) {
				if (temporalExtent.containsKey("timePeriod")) {
					...
					final List positions = (LinkedList) temporalExtent
							.get("timePositions");
					element("gml:timePosition", ((LinkedList) positions)
							.getFirst().toString());
					element("gml:timePosition", ((LinkedList) positions)
							.getLast().toString());
				}
		...
		private void handleContentMetadata(WCS config) {
			...
			List coverages = new ArrayList(config.getData().getCoverageInfos()
					.values());
		...
		private void handleCoverageOfferingBrief(WCS config, CoverageInfo cv) {
			if (cv.isEnabled()) {
				...
				handleEnvelope(cv.getWGS84LonLatEnvelope(), cv
						.getVerticalExtent(), cv.getTemporalExtent());
