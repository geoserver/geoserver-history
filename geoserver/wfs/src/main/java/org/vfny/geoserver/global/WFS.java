...
/**
 * WFS<p>Represents the GeoServer information required to configure an
 ...
 * SessionContext.</p>
 *  <p>WFS wfs = new WFS(dto); System.out.println(wfs.getName());
 * System.out.println(wfs.getAbstract());</p>
 *
...
public class WFS extends Service {
    ...
    /**
     * load purpose.<p>Loads a new data set into this object.</p>
     *
     * @param config
     */
    ...
    /**
     * Gets the prefix for the srs name, based on the SrsXmlStyle
     * property.  If srsXmlStyle is <tt>true</tt> then it is of the xml style,
     * if false then it is EPSG: style.  More apps seem to like the EPSG:
     * style, but the specs seem to lean more to the xml type.  The xml style
     * should actually be more complete, with online lookups for the URI's,
     * but I've seen no real online srs services.
     *
