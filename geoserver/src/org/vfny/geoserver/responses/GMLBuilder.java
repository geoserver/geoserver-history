 ...
 * 
 * <p>
 * This class acts as a "smart" string buffer in which to hold the final output
 * and spends most of its time calling methods of member classes.  Note that
 * this class does not guarantee that the created GML will be either valid or
 * well formed.  Enclosing classes must call <code>GMLBuilder</code> methods
 * in the correct sequence and with correct inputs in order to guarantee valid
 * GML output.  The GMLBuilder class simply creates a convenient shell with
 * its own internal common XML to assist with the GML generation process; it
 * is not a replacement for a thoughtful GetFeature class.
 * </p>
 * 
 * <p>
 * The member classes inside GMLBuilder all correspond to specific GML
 * elements:
 * 
 * <ul>
 * <li>
 * <code>FeatureTypeWriter</code>: feature collection (type)
 * </li>
 * <li>
 * <code>MemberWriter</code>: feature member
 * </li>
 * <li>
 * <code>AttributeWriter</code>: generic schema attribute (ie. W3C schema
 * simple element of any type)
 * </li>
 * <li>
 * <code>GeometryWriter</code>: OGC geometry type (including collections)
 * </li>
 * </ul>
 * </p>
 *
 ...
 * @version $VERSION$
 *
 * @task TODO: Throw this class out, it's a mess.  Use a visitor,  do it in
 *       geotools.
 */
public class GMLBuilder {
    ...
    private FeatureMemberWriter featureMemberWriter = new FeatureMemberWriter();
    ...
    //private Writer finalResult;
    public GMLBuilder(boolean verbose) {
        this(verbose, new StringWriter(20000));
    }
     ...
     */
    public GMLBuilder(boolean verbose, Writer writer) {
        this.verbose = verbose;

        //this.finalResult = writer;
     ...
     * @param lockId the lockId if one exists.  If null then the lockId element
     *        is not used.
     *
     * @task HACK: more than one type should be allowed in a single feature
     *       collection - we have some other hacks, but all are  less than
     *       ideal.
     */
    ...
    /**
     * Adds the featureType to the current feature collection.  This currently
     * only adds it to the xmlns to get the right schema,  so that
     * DescribeFeatureType calls all the type names.  This is obviously less
     * than ideal, as typeNames in different namespaces won't really work
     * together that well.
     *
     ...
     */
    public void initializeGeometry(Class geometry, String srs, String tagName) {
        geometryWriter.initializeGeometry(geometry, srs, tagName);
    }

    /**
     * Return final GML object.
     *
     * @return the gml string built by this class.
     */
  ...
  //J+
    ...
    private void writeString(String s) {
        ...
        /**
         * try { finalResult.write(s); }catch (IOException ex) {}
         */
    }
    ...
    private class FeatureTypeWriter {
         ...
         * @param typeInfo The information on the type to be encoded.
         * @param lockId The lockId for getFeatureWithLock.
         */
        public void start(String srs, TypeInfo typeInfo, String lockId) {
            ...
            writeString(FEATURE_COLL_HEAD);

            if (lockId != null) {
                writeString(indent + "lockId=\"" + lockId + "\"");
            }

            writeString(indent + "xmlns=\"" + xmlns + "\"");
            writeString(indent + XMLNS_GML);
            writeString(indent + XMLNS_WFS);
            writeString(indent + XS_NAMESPACE);
            writeString(indent + "xs:schemaLocation=\"" + xmlns + " ");
        ...
        public void end() {
            ...
            writeString("</wfs:FeatureCollection>");
        }
    }
    ...
    private class FeatureMemberWriter {
        ...
        public void initialize(String featureType) {
            if (verbose) {
                ...
                featureMemberStart1 = "<gml:featureMember><" + featureType
                    + " fid=\"";
                featureMemberStart2 = "\">";
                featureMemberEnd = "</" + featureType + "></gml:featureMember>";
            }
        }
        ...
        public void start(String fid) {
            writeString(featureMemberStart1);
            writeString(fid);
            writeString(featureMemberStart2);
        }
        ...
        public void write(String name, String value) {
            ...
            if ((value != null) && !value.equals("")) {
                ...
                if (value.equals("")) {
                    ...
                    writeString(" xs:nil=\"true\"");
                }

                writeString(attribute2);
                writeString(value);
                writeString(attribute3);
                writeString(name);
                writeString(attribute4);
            }
    ...
    private class GeometryWriter {
         ...
         * initialized geometry and subsequent calls do not match, a run  time
         * exception will be thrown, since geometry type is not checked  by
         * internal methods.
         *
         ...
         */
        private void initializeGeometry(Class geometry, String srs,
            String tagName) {
            String geometryName = "";
            ...
            if (verbose) {
                ...
                // the start tags for the geometry (up to coordinates)
                abstractGeometryStart1 = "<" + tagName + "><gml:"
                    + geometryName + " gid=\"";
                abstractGeometryStart2 = "\" srsName=\"http://www.opengis.net"
                    + "/gml/srs/epsg.xml#" + srs + "\">";

                // the start tags for the geometry (up to coordinates)
                abstractGeometryEnd = "</gml:" + geometryName + "></" + tagName
                    + ">";
        ...
        /**
         * writes the a gml:box, using the coordinates of the passed in
         * geometry.
         *
         * @param geometry should be a polygon of the bounding box to write.
         ...
         *       the bounded by on our own.  If the user chooses to set it in
         *       postgis we can use that, but if they don't we can leave
         *       option in info.xml to print boundedBy automatically, where we
         *       make an envelope for each item.
         */
        private void writeBox(Geometry geometry, String gid) {
            writeString(abstractGeometryStart1 + gid + abstractGeometryStart2);
            writeCoordinates(geometry);
            writeString(abstractGeometryEnd);
        }
        ...
        private void writePoint(Point geometry) {
            if (verbose) {
                writeString(GEOM_OFFSET);
            }

            writeString("<gml:" + geometry.getGeometryType() + ">");
            writeCoordinates(geometry);
         ...
         * @param gid the unique geometry id.
         */
        private void writeMultiLineString(GeometryCollection geometry,
            ...
            for (int i = 0, n = geometry.getNumGeometries(); i < n; i++) {
                ...
                writeString("<gml:lineStringMember>");
        ...
        private void writePolygon(Polygon geometry, String gid) {
            if (verbose) {
                writeString(abstractGeometryStart1 + gid
                    + abstractGeometryStart2);
                writeString(GEOM_OFFSET + "<gml:outerBoundaryIs>" + GEOM_OFFSET
                    + " <gml:LinearRing>");
                writeCoordinates(geometry.getExteriorRing());
                writeString(GEOM_OFFSET + " </gml:LinearRing>" + GEOM_OFFSET
                    + "</gml:outerBoundaryIs>");
                ...
                writeString(abstractGeometryStart1 + gid
                    + abstractGeometryStart2);
                writeString("<gml:outerBoundaryIs><gml:LinearRing" + ">");
                writeCoordinates(geometry.getExteriorRing());
                writeString("</gml:LinearRing></gml:" + "outerBoundaryIs>");

                if (geometry.getNumInteriorRing() > 0) {
                    for (int i = 0; i < geometry.getNumInteriorRing(); i++) {
                        writeString("<gml:innerBoundaryIs>");
                        writeString("<gml:LinearRing>");
                        writeCoordinates(geometry.getInteriorRingN(i));
                        writeString("</gml:LinearRing>");
                        writeString("</gml:innerBoundaryIs>");
                    }
         ...
         * @param gid Geometric ID
         */
        private void writeMultiPolygon(GeometryCollection geometry, String gid) {
            writeString(abstractGeometryStart1 + gid + abstractGeometryStart2);
         ...
         * @param gid Geometric ID
         */
        private void writeMultiGeometry(GeometryCollection geometry, String gid) {
            writeString(abstractGeometryStart1 + gid + abstractGeometryStart2);
        ...
        private void writeCoordinates(Geometry geometry) {
            ...
            for (int i = 0, n = geometry.getNumPoints(); i < n; i++) {
                ...
                writeString(xCoord);
                writeString(coordinateDelimeter);
                writeString(yCoord);

                if (i < (n - 1)) {
                    writeString(tupleDelimeter);
                }
            }

            //finalResult.deleteCharAt(finalResult.length() - 1);
            writeString(coordinatesEnd);
        }
    }
}
