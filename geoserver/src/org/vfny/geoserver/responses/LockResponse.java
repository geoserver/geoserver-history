...
public class LockResponse {
	...
	if (curLock == null) {
	    ...
	    lockId = repository.lock(curTypeName, request.getExpiry());
	    lockedList.add(curTypeName);
		
	    for(int i = 1, n = locks.size(); i < n; i++) {  
		curLock = (LockRequest.Lock)locks.get(i);
		curTypeName = curLock.getFeatureType();
		checkLock(curLock);
		repository.lock(curTypeName, lockId);
		lockedList.add(curTypeName);
	    }
	}
	return generateXml(lockId);
    }

    private static String generateXml(String lockId){
	String indent = "   ";
	...
	returnXml.append(indent + "xsi:schemaLocation=\"http://www.opengis" +
			 ".net/ogc ../wfs/1.0.0/WFS-transaction.xsd\">\n");
