 ...
 * spends most of its time calling methods of member classes.  Note that this class does
 * not guarantee that the created GML will be either valid or well formed.  Enclosing classes
 * must call <code>GMLBuilder</code> methods in the correct sequence and with correct
 * inputs in order to guarantee valid GML output.  The GMLBuilder class simply creates
 * a convenient shell with its own internal common XML to assist with the GML generation
 * process; it is not a replacement for a thoughtful GetFeature class.</p>
 *
 * <p>The member classes inside GMLBuilder all correspond to specific GML elements:<ul>
 * <li><code>FeatureTypeWriter</code>: feature collection (type)
 * <li><code>MemberWriter</code>: feature member
 * <li><code>AttributeWriter</code>: generic schema attribute (ie. W3C schema simple element of any type)
...
public class GMLBuilder {
    ...
    private Category _log = Category.getInstance(GMLBuilder.class.getName());
     ...
     * @param bbox The SRS name.
     */ 
    public void startFeatureType (String featureType, String srs, BoundingBox bbox) {
        ...
        featureTypeWriter.start(srs, bbox);        
    }
    ...
    /**
     * Initialize geometry object.  This method should be called once, before addGeometry.
     *
     ...
     */
    public void initializeGeometry(Class geometry, String featureType, String srs, String tagName) {
    ...
    private class FeatureTypeWriter {
        ...
        /**
         * Writes the feature type start tag, with differing levels of verbosity.
         *
         * @param srs Spactial reference system for the bounding box
         * @param bbox Bounding box for the feature type
         */ 
        public void start(String srs, BoundingBox bbox) {
            
            if( verbose ) {
                finalResult.append("\n<gml:featureCollection xmlns:gml=\"http://www.opengis.net/gml\" scope=\"" + configurationInfo.getUrl() + "\">");
                ...
                finalResult.append("<?xml version='1.0' encoding='UTF-8'?>");
                finalResult.append("<gml:featureCollection xmlns:gml=\"http://www.opengis.net/gml\" scope=\"" + configurationInfo.getUrl() + "\">");
    ...
    private class FeatureMemberWriter {
        ...
        public void start( String fid ) {
            finalResult.append( featureMemberStart1 ).append( fid ).append( featureMemberStart2 );
        }
    ...
    private class AttributeWriter {
        ...
        public void write( String name, String value ) {
            ...
            finalResult.append( attribute1 ).append( name ).append( attribute2 ).append( value ).append( attribute3 ).append( name ).append( attribute4 );
        }
    ...
    private class GeometryWriter {
        ...
        /**
         * Initializes a specific geometry for later writing.  If the initialized geometry and
         * subsequent calls do not match, a run time exception will be thrown, since geometry
         * type is not checked by internal methods.
         *
         ...
         */ 
        private void initializeGeometry(Class geometry, String featureType, String srs, String tagName) { 

            String geometryName = "";
            
            _log.debug("checking type: " + geometry.toString());

            // set internal geometry representation
            if( geometry.equals(Point.class) ) {
                _log.debug("found point");
                geometryType = POINT;
                geometryName = "Point";
            }
            else if( geometry.equals(LineString.class) ) {
                _log.debug("found linestring");
                geometryType = LINESTRING;
                geometryName = "LineString";
            }
            else if( geometry.equals(Polygon.class) ) {
                _log.debug("found polygon");
                geometryType = POLYGON;
                geometryName = "Polygon";
            }
            else if( geometry.equals(MultiPoint.class) ) {
                _log.debug("found multi");
            ...
            if( verbose ) {
                ...
                abstractGeometryStart2 = "\" srsName=\"http://www.opengis.net/gml/srs/epsg.xml#" + srs + "\">\n     ";
                
                // post-coordinate end tags
                abstractGeometryEnd = "\n    </gml:" + geometryName + ">\n   </" + featureType + "." + tagName + ">";
                
                // coordinates start tags
                coordinatesStart = "<gml:coordinates decimal=\".\" cs=\",\" ts=\" \">";				
                ...
                // the start tags for the geometry (up to coordinates)
                abstractGeometryStart1 = "<" + featureType + "." + tagName + "><gml:" + geometryName + " gid=\"";
                abstractGeometryStart2 = "\" srsName=\"http://www.opengis.net/gml/srs/epsg.xml#" + srs + "\">";
                
                // the start tags for the geometry (up to coordinates)
                abstractGeometryEnd = "</gml:" + geometryName + "></" + featureType + "." + tagName + ">";
        ...
        private void writePoint(Point geometry, String gid) {
            ...
            finalResult.append( abstractGeometryStart1 + gid + abstractGeometryStart2 );
         ...
         */ 
        private void writeMultiLineString(GeometryCollection geometry, String gid) {
        ...
        private void writeCoordinates(Geometry geometry) {
            ...
            for( int i = 0 ; i < geometry.getNumPoints() ; i++ ) {
                finalResult.append( tempCoordinates[i].x + coordinateDelimeter + tempCoordinates[i].y + tupleDelimeter );
						}
