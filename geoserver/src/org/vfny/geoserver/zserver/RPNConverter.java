 ...
 * @version $Id: RPNConverter.java,v 1.7.6.1 2003/12/30 23:00:40 dmzwiers Exp $
 */
public class RPNConverter {
     ...
     *
     * @return The lucene representation of the rpnQuery
     *
     ...
     *
     * @param complexQuery the internal jzkit representation of a z39.50 query
     *
     * @return The lucene representation of the complexQuery
     *
     * @throws SearchException If the search type is not supported.
     */
    ...
    /**
     * Helper function for the base case of the recursive conversion.  Turns a
     * jzkit AttrPlusTermNode into a lucene TermQuery.
     *
     * @param rpnTermNode the internal jzkit representation of a  z39.50 term
     *        and Attribute.
     *
     * @return The lucene representation of the rpnTermNode.
     *
    ...
    private Query convertTermNode(AttrPlusTermNode rpnTermNode)
        ...
        if (matchAll
                && (useVal.equals(GeoProfile.Attribute.ANY)
    ...
    /**
     * Creates the query for a date field.  Can take one or two dates in  the
     * value string, seperated by a space.  If there are more than  two dates
     * it just uses the first two.
     *
    ...
    /**
     * If the searchDate is not a full CCYYMMDD string this appends 9's to the
     * end of it.  This is for border cases such as before or during 1996,
     * when just using 1996 as a search term will not pick up all the dates in
     * 1996.  Not needed for the opposite case, such as before 1996, when we
     * actually want to use just 1996, as it is below any specific date in
     * 1996, and also will not match with just 1996.
     *
    ...
    /**
     * Given a string strips out the characters that aren't parts of a number
     * so that the string can be properly converted.
     *
    ...
    /**
     * If the given value is a date or number formats it to the proper  lucene
     * searchable string.  If not just returns the value.
     *
     * @param searchField to be formatted to a date or number.
     * @param searchValue should contain the fgdc metadata xml name of this
     *        value.
     * @param relation The int representation of the comparison to perform.
     * @param truncation Whether the search term should be truncated.
     *
    ...
    private Query getQuery(String searchField, String searchValue,
        ...
        if (GeoProfile.isBoundingField(searchField)) {
            ...
            default:
                returnQuery = new TermQuery(numTerm); //default equals.
    ...
    /**
     * Helper function to turn a string into the lucene number format.  Calls
     * cleanNumber to parse out mistypings such as 9d2.34, returns null if
     * there are no numbers
     *
     * @param searchValue The string representation of a number.
     *
     * @return The NumericField number representation.
     */
