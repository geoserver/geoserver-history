...
public class GeoIndexer {
     ...
     *
     * @param serverProps should contain datafolder, 
     * fieldmap and database keys.
     */
	...
	try {
		...
		for (int i = 0; i < dirFiles.length; i++) {
			...
			if (hits != null && hits.length() > 0) {
			    ...
			    long docModified = 
				DateField.stringToTime
				(hits.doc(0).get(MODIFIED_FIELD));
				...
				docsToAdd.add
				    (XMLDocument.Document(curFile, attrMap));
	    ...
	    LOGGER.warning("The datafolder server property : " 
			   + dataFolder + " is not valid");
	    return 0;
	}
     ...
     * stored documents to see if any should be added or deleted, and then 
     * adds the appropriate documents.  Also creates a new index if needed.
