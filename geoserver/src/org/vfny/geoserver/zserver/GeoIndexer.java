...
/**
 * Performs the indexing of the documents.  The Properties files passed
 * in should have the values 'database' (where the index to be written to
 * is stored) 'datafolder' (where the metadata folders are stored) and 
 * 'fieldmap' (where the mapping of use attribute numbers to xpaths is).  
 * This class has one public function, update, which checks if there
 * are new or modified files in the datafolder, and if so runs the
 * indexer on them.  
 *
...
public class GeoIndexer {
     ...
     *
     * @param serverProps should contain datafolder, fieldmap and database keys.
     */
	...
	if (dataFolder.isDirectory()) {
	    ...
	    File[] dirFiles = dataFolder.listFiles();
		   ...
		   LOGGER.info("No index found at: " + pathToIndex + ".  Creating new index" );
	       }		
			...
			LOGGER.finer("search term is " + searchTerm + " query is " + searchQuery);
			...
			if (hits != null && hits.length() > 0) {
				...
				LOGGER.finer("file at " + path + "has been modified recently");
				...
				LOGGER.info("adding path " + path + " to currentFiles");
				currentFiles.add(path);
			    }
			} else {
	    ...
	    LOGGER.warning("The datafolder server property : " + dataFolder + " is not valid");
	    return 0;
	}
    ...
    /**
     * Helper method to update.  Handles iterating through currently stored documents
     * to see if any should be added or deleted, and then adds the appropriate 
     * documents.  Also creates a new index if needed.
