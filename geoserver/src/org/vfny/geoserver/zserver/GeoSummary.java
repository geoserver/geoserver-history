...
/**
 * Helper class that represents a summary response.  It can return itself as
 * text, html or xml.  To use it simply set the values of the fields with the
 * accessor methods.  A convenience constructor is also provided,  which takes
 * a lucene document and the attribute map used to generate it and fills in
 * the fields that it can.
 *
 * @author Chris Holmes, TOPP
 * @version $Id: GeoSummary.java,v 1.3.6.1 2003/12/30 23:00:40 dmzwiers Exp $
 */
public class GeoSummary {
     ...
     * use attributes.  The method relies on the GeoProfile class to access
     * the  attribute numbers of the fields, using the passed in attrMap to
     * map to  the fields of the lucene document.
     *
     * @param doc The lucene document used to fill the summary fields.
     * @param attrMap The mapping of use attribute numbers to their lucene
     *        field names
     */
    public GeoSummary(org.apache.lucene.document.Document doc,
        ...
        extent = GeoProfile.computeExtent(eastbc, westbc, northbc, southbc)
                           .toString();
    }
     ...
     */
    public void setBrowseGraphic(String fileName, String fileDesc,
        String fileType) {
        browseName = fileName;
        browseDesc = fileDesc;
        browseType = fileType;
     ...
     */
    public void setBounds(String westbc, String eastbc, String northbc,
        String southbc) {
        this.westbc = westbc;
        this.eastbc = eastbc;
        this.northbc = northbc;
        this.southbc = southbc;
        this.extent = GeoProfile.computeExtent(eastbc, westbc, northbc, southbc)
                                .toString();
    }
     ...
     *
     * @return a string of the fieldname, an equals sign, the fieldVal, and a
     *         newline char.
     */
     ...
     * @param fieldVal what the field equals.
     *
     * @return a string of a P tag, the fieldname, an equals sign, the
     *         fieldVal, and a closing P tag, and a newline char.
     */
     ...
     * Currently uses the same return values as the Isite reference
     * implementation, since this is for the fgdc.  The Geo Profile specifies
     * different values, but this is easily modified to use those.
     *
    ...
    public String getHtmlSummary() {
        ...
        + addHtmlField("GEOFORM", geoform) + addHtmlField("BEGDATE", begDate)
        + addHtmlField("ENDDATE", endDate) + addHtmlField("UPDATE", update)
        + addHtmlField("WESTBC", westbc) + addHtmlField("EASTBC", eastbc)
        + addHtmlField("NORTHBC", northbc) + addHtmlField("SOUTHBC", southbc)
        + addHtmlField("ONLINK", onlink) + addHtmlField("INDSPREF", indspref)
    ...
    public String getTextSummary() {
        String retString = addTextField("TITLE", title)
            ...
            + addTextField("ENDDATE", endDate) + addTextField("UPDATE", update)
            + addTextField("WESTBC", westbc) + addTextField("EASTBC", eastbc)
            + addTextField("NORTHBC", northbc)
            + addTextField("SOUTHBC", southbc) + addTextField("ONLINK", onlink)
            + addTextField("INDSPREF", indspref) //addTextField("EXTENT", extent) + 
    ...
    /**
     * Helper function for xml, creates a new text element with the given
     * parent and value.
     *
     ...
     */
    private void addTextElement(Document d, Element parent, String elem_name,
        Object value) {
        if (value != null) {
    ...
    /**
     * Creates an xml summary.   Currently uses the same return values as the
     * Isite reference implementation, since this is for the fgdc.  The Geo
     * Profile specifies different values, but changing this to use those
     * won't be trivial.
     *
     * @return A summary in xml.
     *
     * @task REVISIT: This is lots of bad hardcoding, needs to be redone.  It
     *       would be nice if we could just take an xml document and filter
     *       out all the elements we don't want, and return that new document.
     *       I'm pretty sure this could be done, but I don't know xml that
     *       well.  Possibly a tree walker where the filter takes a list of
     *       xpaths and checks each node to see if its name matches any of the
     *       parts of the xpath.  But I had no time to figure it all out and
     *       get it working.
     */
    public Document getXmlSummary() {
        ...
        try {
            ...
            if (notEmpty(northbc) || notEmpty(eastbc) || notEmpty(westbc)
                    || notEmpty(southbc)) {
            ...
            LOGGER.severe("exception with the parser, check the xerces jars: "
                + e.getMessage());
        }
