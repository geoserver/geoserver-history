...
public class EncodeSVG {
	...
	public String createViewBox() {
		...
		String viewBox = writer.getX(referenceSpace.getMinX())
				+ " "
				+ (writer.getY(referenceSpace.getMinY()) - referenceSpace
						.getHeight()) + " " + referenceSpace.getWidth() + " "
	...
	private void writeHeader() throws IOException {
		...
		header = header.replaceAll("_width_", String.valueOf(mapContext
				.getMapWidth()));
		header = header.replaceAll("_height_", String.valueOf(mapContext
				.getMapHeight()));
		writer.write(header);
	}
	...
	private void writeLayers() throws IOException, AbortedException {
		...
		for (int i = 0; i < nLayers; i++) {
			...
			try {
				Expression bboxExpression = fFac
						.createBBoxExpression(mapContext.getAreaOfInterest());
