 ...
 *
 */
package org.vfny.geoserver.wms.gtserver;
...
public class GtWmsServer implements WMSServer {
    ...
    java.util.Map layerEntries = new HashMap();
    java.util.Map features = new HashMap();
    ...
    private void loadLayers(String filename) {
		...
		//No longer using the layer.xml file for geoserver.  Can
		//bring this back later if desired...  But we probably
		//should be aiming to get rid of that configuration...
		//Instead just put the relevant info in info.xml
	    ...
	    ConfigInfo.getInstance(); //make sure things have been 
	    //initialized.
	    TypeRepository repo = TypeRepository.getInstance();
	    for (Iterator iter = repo.getTypes(); iter.hasNext();){
		LayerEntry curLayer = ((TypeInfo)iter.next()).asWMSLayer();
		LOGGER.fine("adding type as layer " + curLayer);
		layerEntries.put(curLayer.id,curLayer);
	    } 
		...
		//Is any of this url stuff necessary?  ch.
                LOGGER.finer("pre mod url is " +
			     entry.properties.get("url"));
		...
		//huh?  Postgis?  It don't have a url.  I'd like to move
		//this to my filename/url paradigm, do this transform earlier.
                try {
                    url = new URL((String)entry.properties.get("url"));
                } catch (MalformedURLException mue) {
                    try{
                        url = new URL(home, (String)entry.properties.get("url"));
			if (url != null) {
			    entry.properties.put("url", url.toExternalForm());
			}
		...
		LOGGER.fine("putting layer " + entry.id + ", " + cache);
...
//            exp.printStackTrace();
    ...
    Color bgcolor) throws WMSException {
        ...
        LOGGER.fine("available : " + features);
    ...
    throws MalformedURLException, WMSException {
        String sldpath = "data/styles/" + (String) layerdefn.styles.get(style);
