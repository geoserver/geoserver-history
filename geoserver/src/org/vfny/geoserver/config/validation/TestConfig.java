	...
	public PropertyDescriptor[] getPropertyDescriptors(){
		if(pds == null){
			...
			for(int i=0;i<completeList.length;i++){
                ...
                if( property.isHidden() ) continue; // only for tool use
                if( property.isExpert() ) continue; // limited to plugin definition
                if( property.getWriteMethod() == null ) continue; // skip read-only properties
                
                if( "name".equals(property.getName() )) continue; // not handled dynamically
                if( "description".equals(property.getName() )) continue; // not handled dynamically
