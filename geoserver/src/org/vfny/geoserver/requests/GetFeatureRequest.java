...
public class GetFeatureRequest {
     ...
     *
     */ 
    public boolean equals(GetFeatureRequest request) {

        boolean isEqual = true;

        _log.info("about to check for equality");
        if( request.getHandle().equals(handle) &&
            request.getOutputFormat().equals(outputFormat) &&
            request.getVersion().equals(version) &&
            (request.getMaxFeatures() == maxFeatures)) {            

            _log.info("passed all basic tests");

            Iterator internalIterator = queries.listIterator();
            Iterator externalIterator = request.getQueries().listIterator();
            while( internalIterator.hasNext()) {
                if( !externalIterator.hasNext()) {
                    isEqual = false;
                    _log.info("queries not same size");
                }
                else {
                    if( ((Query) internalIterator.next()).equals((Query) externalIterator.next())) {
                        isEqual = true && isEqual;
                        _log.info("queries property tests");
                    }
                    else {
                        isEqual = false;
                        _log.info("queries not equal");
                    }
                }
            }
        }
