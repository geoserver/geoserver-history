...
public class DijkstraTraversal extends SourceGraphTraversal {
  ...
  public DijkstraTraversal(
    Graph graph, GraphWalker walker, GraphComponent source, CostFunction cf
  ) {
    super(graph, walker, source);
    m_cf = cf;
    m_processed = new Heap(
      new Comparator() {
        public int compare(Object o1, Object o2) {
          CostNode n1 = (CostNode)o1;
          CostNode n2 = (CostNode)o2;
            
          if (n1.m_cost < n2.m_cost) return(-1);
          if (n1.m_cost > n2.m_cost) return(1);
          return(0);
        }
      }
  ...
  protected void walk() {
    while(!m_processed.isEmpty()) {
      ...
      if (getWalker().visit(current.m_element, this) == STOP) return;
      
      for (
        Iterator itr = current.m_element.getAdjacentElements().iterator(); 
        itr.hasNext();
      ) {
        GraphComponent adjacent = (GraphComponent)itr.next();
        if (getWalker().isVisited(adjacent)) continue;
        
        CostNode cn = (CostNode)m_ge2costNode.get(adjacent);
        double cost = current.m_cost + m_cf.getCost(current.m_element, adjacent);
  ...
  /**
   * Represents a cost function to be used by dijkstras algorithm to calculate
   * node costs.
   */
  public interface CostFunction {
    public double getCost(GraphComponent srcElement, GraphComponent destElement);  
  }
