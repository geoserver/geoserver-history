 ...
 * that are to be performed on this FeatureTypeConfig.
 * <p>
 ...
 * <p>
 * For validations that are performed on every FeatureTypeConfig, a value called ANYTYPENAME
 ...
 * <p>
 * processor.runFeatureTests(FeatureTypeConfig, Feature, ValidationResults);<br>
 * processor.runIntegrityTests(layers, Envelope, ValidationResults);<br>
 * </code></pre>
 * 
 * @author bowens, Refractions Research, Inc.
 * @author $Author: dmzwiers $ (last modification)
 * @version $Id: ValidationProcessor.java,v 1.2.2.1 2003/12/31 23:36:44 dmzwiers Exp $
 */
public class ValidationProcessor {
	...
	private void addToFVLookup(FeatureValidation validation) 
	{
		...
		else	// a non ALL FeatureTypeConfig validation
		{
			for (int i=0; i<featureTypeList.length; i++)
			{
				ArrayList tests = (ArrayList)featureLookup.get(featureTypeList[i]);
				if (tests == null)	// if this FeatureTypeConfig doesn't have a validation test yet
	 ...
	 * <p>
	 * Gets all the FeatureTypes that this FeatureTypeConfig uses. 
	 * </p>
	 * @param typeName the FeatureTypeName
	 * @return all the FeatureTypes that this FeatureTypeConfig uses.
	 */
	public Set getDependencies(String typeName)
	{
		...
		if (validations != null)
		{
			for (int i=0; i<validations.size(); i++)	// for each validation
			{
				...
				for (int j=0; j<types.length; j++)	// for each FeatureTypeConfig
	 ...
	 * runFeatureTests
	 * <p>
	 * Performs a lookup on the FeatureTypeConfig name to determine what FeatureTests
	 ...
	 * to be performed on every FeatureTypeConfig. An example of this could be an isValid() test
	 * on all geometries in all FeatureTypes. Once those tests have been gathered,
	 * a lookup is performed on the TypeName of the FeatureTypeConfig to check for
	 * specific FeatureTypeConfig validation tests.
	 ...
	 * </p>
	 * @param type The FeatureTypeConfig of the features being tested.
	 * @param collection The collection of features, of a particulare FeatureTypeConfig "type", that are to be validated.
	...
	public void runFeatureTests(FeatureType type, FeatureCollection collection, ValidationResults results) throws Exception
	{
		...
		// check for any FeatureTypeConfig specific tests
		...
		if (tests != null)	// if we found some tests to be performed on this FeatureTypeConfig
		{
	 ...
	 * runIntegrityTests
	 * <p>
	 * Performs a lookup on the FeatureTypeConfig name to determine what IntegrityTests
	 ...
	 * The first validation test lookup checks to see if there are any validations that are
	 * to be performed on every FeatureTypeConfig. An example of this could be a uniqueID() test
	...
	public void runIntegrityTests(Map stores, Envelope envelope, ValidationResults results) throws Exception
	{
		...
		// for each modified FeatureTypeConfig
