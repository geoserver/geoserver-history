 ...
 * that are to be performed on this GlobalFeatureType.
 * <p>
 ...
 * <p>
 * For validations that are performed on every GlobalFeatureType, a value called ANYTYPENAME
 ...
 * <p>
 * processor.runFeatureTests(GlobalFeatureType, Feature, ValidationResults);<br>
 * processor.runIntegrityTests(layers, Envelope, ValidationResults);<br>
 * </code></pre>
 * 
 * @author bowens, Refractions Research, Inc.
 * @author $Author: dmzwiers $ (last modification)
 * @version $Id: ValidationProcessor.java,v 1.2.2.2 2004/01/03 00:20:14 dmzwiers Exp $
 */
public class ValidationProcessor {
	...
	private void addToFVLookup(FeatureValidation validation) 
	{
		...
		else	// a non ALL GlobalFeatureType validation
		{
			for (int i=0; i<featureTypeList.length; i++)
			{
				ArrayList tests = (ArrayList)featureLookup.get(featureTypeList[i]);
				if (tests == null)	// if this GlobalFeatureType doesn't have a validation test yet
	 ...
	 * <p>
	 * Gets all the FeatureTypes that this GlobalFeatureType uses. 
	 * </p>
	 * @param typeName the FeatureTypeName
	 * @return all the FeatureTypes that this GlobalFeatureType uses.
	 */
	public Set getDependencies(String typeName)
	{
		...
		if (validations != null)
		{
			for (int i=0; i<validations.size(); i++)	// for each validation
			{
				...
				for (int j=0; j<types.length; j++)	// for each GlobalFeatureType
	 ...
	 * runFeatureTests
	 * <p>
	 * Performs a lookup on the GlobalFeatureType name to determine what FeatureTests
	 ...
	 * to be performed on every GlobalFeatureType. An example of this could be an isValid() test
	 * on all geometries in all FeatureTypes. Once those tests have been gathered,
	 * a lookup is performed on the TypeName of the GlobalFeatureType to check for
	 * specific GlobalFeatureType validation tests.
	 ...
	 * </p>
	 * @param type The GlobalFeatureType of the features being tested.
	 * @param collection The collection of features, of a particulare GlobalFeatureType "type", that are to be validated.
	...
	public void runFeatureTests(FeatureType type, FeatureCollection collection, ValidationResults results) throws Exception
	{
		...
		// check for any GlobalFeatureType specific tests
		...
		if (tests != null)	// if we found some tests to be performed on this GlobalFeatureType
		{
	 ...
	 * runIntegrityTests
	 * <p>
	 * Performs a lookup on the GlobalFeatureType name to determine what IntegrityTests
	 ...
	 * The first validation test lookup checks to see if there are any validations that are
	 * to be performed on every GlobalFeatureType. An example of this could be a uniqueID() test
	...
	public void runIntegrityTests(Map stores, Envelope envelope, ValidationResults results) throws Exception
	{
		...
		// for each modified GlobalFeatureType
