GEOSERVER 1.5+ DATA STORE EXTRAS README

This package contains an WFS DataStore implementation that is 
distributed as a separate plug-in.  This plug-in is still experimental,
if you have feedback please let us know.  See 
http://docs.codehaus.org/display/GEOSDOC/WFS+DataStore for the latest
information.  

Please report any bugs with jira (http://jira.codehaus.org/browse/GEOS). 

Any other issues can be discussed on the mailing list (http://lists.sourceforge.net/lists/listinfo/geoserver-users).

Contains the following data stores:

INSTALLATION

1. Copy included gt2-wfs jar in to your GeoServer library directory.  In
   a binary install this is [GEOSERVER_HOME]/server/geoserver/WEB-INF/lib/
   In a war install this is [container]/webapps/geoserver/WEB-INF/lib/

2. Restart GeoServer.

COMPATIBILITY

This jar should work with any version of GeoServer based on GeoTools 2.2.x.  
Currently this is anything in 1.5.x.
