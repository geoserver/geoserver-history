...
/**
 * Base class that converts back and forth between varied data formats and a Java Map to simplify
 * the implementation of a RESTful web API.
 * 
...
public abstract class MapResource extends Resource {
    ...
    /**
     * This method should return a map of format name == DataFormat for the MapResource to use in
     * translating to and from different output Formats
     */
    ...
    public void handleGet() {
        try {
            ...
            if ((myRequestFormat == null) | (details == null)) {
                LOG.info("Failed MapResource request; format: " + myRequestFormat + "; details: "
                        + details);
    ...
    /**
     * This method must be overridden by subclasses; it will be called to handle the HTTP GET
     * method.
     * 
     * @param details
    ...
    /**
     * Put some metadata about the HTTP location of the resource into a map so that it can be made
     * available to the DataFormat.
     * 
    ...
    public Map getPageDetails() {
        ...
        currentURL = RequestUtils.proxifiedBaseURL(currentURL, myGeoserver.getProxyBaseUrl());
    ...
    public void handlePut() {
        try {
            myRequestFormat = (DataFormat) myFormatMap
                    .get(getRequest().getAttributes().get("type"));

            Object details = myRequestFormat.readRepresentation(getRequest().getEntity());

            if ((myRequestFormat == null) || (details == null)) {
                throw new RestletException(new StringRepresentation(
                        "Could not find  requested resource", MediaType.TEXT_PLAIN),
                        Status.CLIENT_ERROR_NOT_FOUND);
            }
     ...
     *                 if anything goes wrong. The Status and Representation in the RestletException
