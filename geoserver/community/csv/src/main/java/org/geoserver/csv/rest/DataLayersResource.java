...
public class DataLayersResource extends AbstractLayersResource {
    ...
    public void handlePost() {
        ...
        try {
            ...
            if(file == null) {
                resp.setEntity("'file' parameter is required but not provided");
                resp.setStatus(Status.CLIENT_ERROR_BAD_REQUEST);
                return;
            } else if(geometryLayer == null) {
                resp.setEntity("'geometryLayer' parameter is required but not provided");
                resp.setStatus(Status.CLIENT_ERROR_BAD_REQUEST);
                return;
            } else if(!csvService.getGeometryLayers().contains(geometryLayer)) {
                resp.setEntity("geometryLayer " + geometryLayer + " is unknown, valid values are " + csvService.getGeometryLayers());
                resp.setStatus(Status.CLIENT_ERROR_BAD_REQUEST);
                return;
            }
             ...
             csvService.configureCsvFile(geometryLayer, "zcta", csv);
        } catch (Exception e) {
            LOGGER.log(Level.SEVERE, "Error occurred managing csv file upload request", e);
