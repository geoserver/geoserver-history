...
public class RulesResource extends BaseResource {
	...
	public RulesResource(Context con, Request req, Response resp, Data data) {
		super(con, req, resp, data);

		this.userStyleID = req.getAttributes().get("userStyleID").toString();
		this.featureTypeStyleID = Integer.parseInt(req.getAttributes().get(
				"featureTypeID").toString().trim());
		if (req.getAttributes().get("firstRuleID") != null)
			this.firstRuleID = Integer.parseInt(req.getAttributes().get(
					"firstRuleID").toString().trim());
		if (req.getAttributes().get("lastRuleID") != null)
			this.lastRuleID = Integer.parseInt(req.getAttributes().get(
	...
	public void handlePost() {
		...
		fTStyle = style.getFeatureTypeStyles()[this.featureTypeStyleID];
		/*
		 * insert new rules, if there is a firstruleID you start inserting from that
		 */
