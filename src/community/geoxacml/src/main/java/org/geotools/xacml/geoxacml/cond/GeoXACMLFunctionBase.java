...
import org.geotools.xacml.geoxacml.cond.GeoXACMLException;
 ...
 * 
 * Also responsible for coordinate transformations.
 * WGS84 is the common CRS, if transformation has to be done, 
 * the target CRS is always WGS84 
 *
 */
public abstract class GeoXACMLFunctionBase extends FunctionBase {
	...
	public GeoXACMLFunctionBase(String functionName, int functionId, String paramType, boolean paramIsBag, int numParams, int minParams, String returnType, boolean returnsBag) {
		super(functionName, functionId, paramType, paramIsBag, numParams, minParams,
				returnType, returnsBag);
	}

	public GeoXACMLFunctionBase(String functionName, int functionId, String paramType, boolean paramIsBag, int numParams, String returnType, boolean returnsBag) {
		super(functionName, functionId, paramType, paramIsBag, numParams, returnType,
				returnsBag);
	}

	public GeoXACMLFunctionBase(String functionName, int functionId, String returnType, boolean returnsBag) {
		super(functionName, functionId, returnType, returnsBag);
	}

	public GeoXACMLFunctionBase(String functionName, int functionId, String[] paramTypes, boolean[] paramIsBag, String returnType, boolean returnsBag) {
		super(functionName, functionId, paramTypes, paramIsBag, returnType, returnsBag);
	}

	/**
	 * @param array	 	array of GeometryAttributes, length == 2
	 ...
	 * 
	 * 1) Both have no srsName (null)
	 * 2) Both have srsNames and  equalsIgnoreCase is true
	 * 3) Both CRS are decodeable and equalsIgnoreMetaData is true
	 * 
	 * Error situations are
	 * 
	 * 1) One attribute has a srsName, the other not
	 * 2) a CRS is not decodeable
	 * 
	...
	protected String transformOnDemand(GeometryAttribute array[]) throws GeoXACMLException{
		...
		if (attr1.getSrsName()==null && attr2.getSrsName()==null) return null;
		...
		try {
			for (int i = 0; i < array.length;i++) {				
				Geometry newGeom = transformToCommonCRS(array[i].getGeometry(), array[i].getSrsName(), crsArray[i]);
				if (newGeom!=array[i].getGeometry()) { // geometry changed
					GeometryAttribute newGeomAttr = new GeometryAttribute(newGeom,COMMON_CRS_NAME,
							array[i].getGid(),array[i].getGmlVersion(),array[i].getType().toString());
	 ...
	 * No transformation in the following situations
	 * 
	 * 1) the srsName equalsIgnoreCase with EPSG:4326 is true
	 * 2) equalsIgnorMetaData returns true
	 */
	protected Geometry transformToCommonCRS(Geometry g, String srsName, CoordinateReferenceSystem sourceCRS) throws GeoXACMLException{
		...
		try {
			...
			if (COMMON_CRS_NAME.equalsIgnoreCase(srsName)) return g;
			
						
			if (CRS.equalsIgnoreMetadata(sourceCRS, COMMON_CRS)) return g;
