...
/**
 * 
 * Class for supporting GML2 encoding and 
 * GML2 Parsing 
 *
...
public class GML3Support extends GMLSupport {
	...
	public final static GML3Support Singleton = new GML3Support();
	 ...
	 GeometryAttribute buildFromGML(Node gmlNode) throws Exception {
			...
			if ("Point".equals(gmlNode.getLocalName())) {
				...
				GeometryAttribute attr = new GeometryAttribute(p, srsName, gid,
						GMLVersion.Version3,GML_POINT);
			...
			if ("LineString".equals(gmlNode.getLocalName())) {
				...
				GeometryAttribute attr = new GeometryAttribute(ls, srsName, gid,
						GMLVersion.Version3,GML_LINESTRING);
				attr.setSrsDimension(srsDimension);
				return attr;
			}
			...
			if ("LinearRing".equals(gmlNode.getLocalName())) {
				...
				GeometryAttribute attr = new GeometryAttribute(lr, srsName, gid,
						GMLVersion.Version3,GML_LINEARRING);
				attr.setSrsDimension(srsDimension);
				return attr;
			}
			...
			if ("Polygon".equals(gmlNode.getLocalName())) {
				...
				GeometryAttribute attr = new GeometryAttribute(poly, srsName, gid,
						GMLVersion.Version3,GML_POLYGON);
				attr.setSrsDimension(srsDimension);
				return attr;
			}
			...
			if ("Envelope".equals(gmlNode.getLocalName())) {
				...
				GeometryAttribute attr = new GeometryAttribute(poly, srsName, gid,
						GMLVersion.Version3,GML_ENVELOPE);
				attr.setSrsDimension(srsDimension);
				return attr;
			}
			...
			if ("MultiPoint".equals(gmlNode.getLocalName())) {
				...
				GeometryAttribute attr = new GeometryAttribute(mp, srsName, gid,
						GMLVersion.Version3,GML_MULTIPOINT);
				attr.setSrsDimension(srsDimension);
				return attr;
			}
			...
			if ("MultiCurve".equals(gmlNode.getLocalName())) {
				...
				GeometryAttribute attr = new GeometryAttribute(mp, srsName, gid,
						GMLVersion.Version3,GML_MULTICURVE);
				attr.setSrsDimension(srsDimension);
				return attr;
			}
			...
			if ("MultiSurface".equals(gmlNode.getLocalName())) {
				...
				GeometryAttribute attr = new GeometryAttribute(mp, srsName, gid,
						GMLVersion.Version3,GML_MULTISURFACE);
				attr.setSrsDimension(srsDimension);
				return attr;
			}
	...
	void encodeASGML (GeometryAttribute attr, StringBuffer buff) {
		...
		if (GML_POINT.equals(attr.getGmlType()))
			encodePoint((Point) attr.getGeometry(), buff,abstractGeometryAttributeMap,attr.getSrsDimension());

		if (GML_LINESTRING.equals(attr.getGmlType()))
			encodeLineString((LineString) attr.getGeometry(), buff,abstractGeometryAttributeMap,attr.getSrsDimension());
		
		if (GML_LINEARRING.equals(attr.getGmlType()))
			encodeLinearRing((LinearRing) attr.getGeometry(), buff,abstractGeometryAttributeMap,attr.getSrsDimension());

		if (GML_POLYGON.equals(attr.getGmlType()))
			encodePolygon((Polygon) attr.getGeometry(), buff,abstractGeometryAttributeMap,attr.getSrsDimension());
		
		if (GML_ENVELOPE.equals(attr.getGmlType()))
			encodeEnvelope((Polygon) attr.getGeometry(), buff,abstractGeometryAttributeMap,attr.getSrsDimension());
		
		if (GML_MULTIPOINT.equals(attr.getGmlType()))
			encodeMultiPoint((MultiPoint) attr.getGeometry(), buff,abstractGeometryAttributeMap,attr.getSrsDimension());
		
		if (GML_MULTICURVE.equals(attr.getGmlType()))
			encodeMultiCurve((MultiLineString) attr.getGeometry(), buff,abstractGeometryAttributeMap,attr.getSrsDimension());
		
		if (GML_MULTISURFACE.equals(attr.getGmlType()))
			encodeMultiSurface((MultiPolygon) attr.getGeometry(), buff,abstractGeometryAttributeMap,attr.getSrsDimension());
	...
	public LinearRing parseLinearRing(Node node, int srsDimension) throws GMLException {
		...
		if (lr == null)
			throw new GMLException("Could not find \"pos\",\"posList\",\"coords\" or \"coordinates\"  for LinearRing");			
		return lr;	
	}
    ...
    protected Coordinate parseDirectPosition( Node node, int srsDimension)
    	throws GMLException {
    ...
    protected void encodeDirectPostion(Coordinate c, StringBuffer buff, int srsDimension) {
    	...
    	buff.append(c.x);
    	if (srsDimension==2) buff.append(" ").append(c.y);
    	if (srsDimension==3) buff.append(" ").append(c.z);
    	encodeEndTag("pos", buff);
    }
    
    protected void encodeDirectPositionList(CoordinateSequence s, StringBuffer buff, int srsDimension) {
    ...
    protected CoordinateSequence parseDirectPositionList(Node node, int srsDimension) throws GMLException {
...
//        }
        ...
        for (int i = 0; i < coordinatesCount; i++) {
        	...
        	if (srsDimension==1) array[i]=new Coordinate(values[offset],Double.NaN,Double.NaN);
        	if (srsDimension==2) array[i]=new Coordinate(values[offset],values[offset+1],Double.NaN);
        	if (srsDimension==3) array[i]=new Coordinate(values[offset],values[offset+1],values[offset+2]);	
        }
	...
	protected void encodePoint(Point p, StringBuffer buff, Map<String,String> abstractGeometryAttributeMap,int srsDimension) {
	...
	public void encodeLineString(LineString ls,StringBuffer buff,Map<String,String> abstractGeometryAttributeMap, int srsDimension) {
	...
	public void encodeLinearRing(LinearRing lr,StringBuffer buff,Map<String,String> abstractGeometryAttributeMap, int srsDimension) {
	...
	public void encodePolygon(Polygon poly, StringBuffer buff,Map<String,String> abstractGeometryAttributeMap, int srsDimension) {
		...
		for (int i =0; i < countInner;i++) {
			...
			encodeStartTag("LinearRing", buff, null);
			encodeDirectPositionList(poly.getInteriorRingN(i).getCoordinateSequence(), buff,srsDimension);
			encodeEndTag("LinearRing", buff);
	...
	public void encodeEnvelope(Polygon poly,StringBuffer buff,Map<String,String> abstractGeometryAttributeMap, int srsDimension) {
	...
	public void encodeMultiPoint(MultiPoint mp, StringBuffer buff,Map<String,String> abstractGeometryAttributeMap, int srsDimension) {
	...
	public void encodeMultiCurve(MultiLineString mls, StringBuffer buff,Map<String,String> abstractGeometryAttributeMap, int srsDimension) {
	...
	public void encodeMultiSurface(MultiPolygon mp, StringBuffer buff,Map<String,String> abstractGeometryAttributeMap, int srsDimension) {
