...
public class XACMLDefaultRoleAuthority implements XACMLRoleAuthority {
    ...
    private static InheritableThreadLocal<Set<Authentication>> AlreadyPrepared = 
        new InheritableThreadLocal<Set<Authentication>>();
    ...
    private void setRoleParamsFromUserDetails(Authentication auth, Obligation obligation, XACMLRole role) {
                ...
                if (pd.getName().equals(propertyName)) {
                    ...
                    if (value instanceof Geometry) {
                        if (((Geometry) value).getUserData()==null)
                            throw new RuntimeException("Property: " + propertyName +" : Geometry must have srs name as userdata");
                    }
    ...
    protected AttributeValue createAttributeValueFromObject(Serializable object) {
        ...
        if (object instanceof Geometry) {
            ...
            try {
                retVal= new GeometryAttribute(g,g.getUserData().toString(),null,GMLVersion.Version3,gmlType);
