...
public class GeoXACMLGeometryTest extends GeoServerTestSupport {
    ...
    public void testRoleAttributes() {
        ...
        XACMLRole readerRole = GeoXACMLConfig.getXACMLRoleAuthority().getRolesFor(reader)
                .iterator().next();
        ...
        for (Attribute attr : subject.getAttributes()) {
            if (attr.getId().toString().equals(
                    XACMLConstants.RoleAttributeId + XACMLConstants.RoleParamIdInfix + "persNr"))
                count++;
            if (attr.getId().toString().equals(
                    XACMLConstants.RoleAttributeId + XACMLConstants.RoleParamIdInfix
                            + "geometryRestriction"))
                count++;
        }
        assertTrue(count == 2);
