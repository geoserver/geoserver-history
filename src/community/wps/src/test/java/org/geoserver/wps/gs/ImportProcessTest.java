...
public class ImportProcessTest extends GeoServerTestSupport {
	...
	/**
	 * Try to reimport buildings as another layer (different name, different
	 * projection)
	 */
	public void testImportBuildings() throws Exception {
		FeatureTypeInfo ti = getCatalog().getFeatureTypeByName(
				getLayerId(MockData.BUILDINGS));
		SimpleFeatureCollection rawSource = (SimpleFeatureCollection) ti
		...
		SimpleFeatureIterator fi = fs.getFeatures(
				ff.equals(ff.property("FID"), ff.literal("113"))).features();
