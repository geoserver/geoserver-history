 ...
 *
 */
public class UniqueIntervalGenerator extends Restlet {
	final private RulesBuilder builder = new RulesBuilder();
	private Data dt;
	
	public UniqueIntervalGenerator(Data dt) {
		this.dt = dt;
	}
	...
	public void handle(Request request, Response response) {
		...
		List<Rule> rules = this.generateUniqueInterval(attributes);
		JSONArray json = null;
	...
	private List<Rule> generateUniqueInterval(Map attributes) {
		/* Looks in attribute map if there is the featureType param */
		if (attributes.containsKey("featureType") && attributes.containsKey("property")) {
			...
			try {
				...
				return builder.uniqueIntervalClassification(ftCollection, property);
