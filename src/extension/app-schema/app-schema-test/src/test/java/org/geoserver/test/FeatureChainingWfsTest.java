...
public class FeatureChainingWfsTest extends AbstractAppSchemaWfsTestSupport {
    ...
    public void testDescribeFeatureType() throws IOException {
        ...
        /**
		 * om:Observation has 2 schemaURIs specified in the mapping file. Both
		 * must appear.
		 */
		doc = getAsDOM("wfs?request=DescribeFeatureType&typename=om:Observation");
		LOGGER
				.info("WFS DescribeFeatureType, typename=om:Observation response:\n"
						+ prettyString(doc));
		assertEquals("xsd:schema", doc.getDocumentElement().getNodeName());
		assertXpathEvaluatesTo(FeatureChainingMockData.OM_URI,
				"//@targetNamespace", doc);
		assertXpathCount(1, "//xsd:include", doc);
