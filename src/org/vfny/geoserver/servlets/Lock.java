 ...
 * @version $Id: Lock.java,v 1.5 2003/09/15 23:07:16 cholmesny Exp $
 */
public class Lock extends HttpServlet {
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        // implements the main request/response logic
        try {
            LockKvpReader currentKvpRequest = new LockKvpReader(request
                    .getQueryString());
            LockRequest wfsRequest = currentKvpRequest.getRequest();
            tempResponse = LockResponse.getXmlResponse(wfsRequest);
        } catch (WfsException wfs) {
            tempResponse = wfs.getXmlResponse();
        } catch (Throwable e) {
        //wraps all errors; client should never see a servlet stack trace.
            WfsException wfse = new WfsException(e, "UNCAUGHT EXCEPTION", null);
            tempResponse = wfse.getXmlResponse(true);
            LOGGER.info("Had an undefined error: " + e.getMessage());
            e.printStackTrace();
