...
/**
 * Implements a way to turn numbers into comparable strings
 * and vice versa, so that they can be indexed and compared by
 * lucene.
 *
 * @author James Ricci
 * @author Chris Holmes, TOPP
 * This code was mostly taken from the lucene users email list:
 * From: James Ricci
 * Subject: RE: Numeric Support
 * Date: Fri, 26 Jul 2002 18:53:49 -0700 
 *
 *@version $VERSION$
 */
...
public class NumericField
{
    ...
    /**
     * Normalizes the digit into a character.  The signMultiplier
     * tells which way to normalize, up for positive, down for negative.
     *
     * @param digit The character of the digit to be fixed up.
     * @param signMultiplier should be 1 for positives, -1 for negatives.
     *
     * @return the char that is 'digit' chars away from the NORM, in
     * the direction of the signMultiplier. */
     ...
     * Simply does the parsing on the string, passes it to numberToString.
     *
    ...
    /**
     * Calls numberToString with default values for the
     * left and right significant digits.  
     *
    ...
    /**
     * Converts a number to a comparable string. Uses a given number
     * of significant digits to the left and right of the decimal.
     *
     * @param num the number to convert.
     ...
     * @return the lucene comparable string.
     */
    private static String numberToString(Double num, int leftPlaces, int
rightPlaces)
    {
	...
	big = big.setScale(rightPlaces, BigDecimal.ROUND_HALF_UP);
        ...
        String wholePart = (decimalPos >= 0) ? 
	    s.substring(0, decimalPos) : s;
        String fracPart  = (decimalPos >= 0) ? 
	    s.substring(decimalPos + 1) : "";
	/*Start of 
    ...
    /**
     * Calls stringToNumber with default values for the left
     * and right significant digits.
     *
    ...
    /**
     * Converts a comparable string to a Double. Uses a given number
     * of significant digits to the left and right of the decimal.
     *
     ...
     * @return the Double represented by s.
     */
    private static Double stringToNumber(String s, int leftPlaces, int
rightPlaces) 
    {
