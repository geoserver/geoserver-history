...
/**
 * This class allows for abstracting the location of the Geoserver Data
 * directory.
 * 
 * <p>
 * Some people call this "GEOSERVER_HOME".  Inside this directory should be two
 * more directories:
 * 
 * <ul>
 * <li>
 ...
 * </li>
 * <li>
 * b. "data/"     Inside this is a set of other directories. For the exact
 * content of these directories, see any existing geoserver install's
 * server/geoserver directory.  In order to find the geoserver data directory
 * the following steps take place: 1. search for the "GEOSERVER_DATA_DIR"
 * system property. this will most likely have come from "java
 * -DGEOSERVER_DATA_DIR=..." or from you web container 2. search for a
 * "GEOSERVER_DATA_DIR" in the web.xml document
 * <pre><code>
 *        <context-param>
 ...
 * 
 * <p>
 * NOTE: a set method is currently undefined because you should either modify
 * you web.xml or set the environment variable and re-start geoserver.
 * </p>
 *
 * @author dblasby
 */
public class GeoserverDataDirectory {
    ...
    /**
     * See the class documentation for more details. 1. search for the
     * "GEOSERVER_DATA_DIR" system property.  2. search for a
     * "GEOSERVER_DATA_DIR" in the web.xml document 3. It defaults to the old
     * behavior - ie. the application root - usually "server/geoserver" in
     * your .WAR.
     *
     * @param servContext DOCUMENT ME!
     *
     ...
     * outside the webapp, or if its defaulting to the webapp embedded
     * dataDir. We're in the process of moving away from storing anything in
     * the webapp but are keeping this to ease the transition.
     *
    ...
    /**
     * Utility method to find the approriate sub-data dir config.  This is  a
     * helper for the fact that we're transitioning away from the WEB-INF type
     * of hacky storage, but during the transition things can be in both
     * places.  So this method takes the root file, the dataDir, and a name of
     * a directory that is stored in the data dir, and checks for it in the
     * data/ dir (the old way), and directly in the dir (the new way)
     *
     ...
     *
     * @throws ConfigurationException if the directory could not be found at
     *         all.
     */
