...
public class MultipleDelegatingTilesRequestProcessor extends DelegatingTilesRequestProcessor {
	...
	throws IOException, ServletException {
		...
		if (processTilesDefinition(forward.getPath(),
				/*forward.getContextRelative()*/false,
				request,
				response)) {
			if (log.isDebugEnabled()) {
				log.debug(
		...
		try {
			...
			if (definitionsFactory != null) {
				...
				if (definition != null) { // We have a definition.
					...
					uri = /*WebApplicationContextUtils.getWebApplicationContext(getServletContext()).getResources(definition.getPath())[0].getURL().getPath()*/
							definition.getPath();
