...
public class PropertyEncodingOrderTest extends AbstractAppSchemaWfsTestSupport {
	...
	/**
	 * Test the gmsl:PlanarOrientation is encoded in the order of aziumth,
	 * convention, dip, polarity according to the schema CGI_Value.xsd
	 * 
	 * @throws Exception
	 */
	public void testPropertyEncodingOrder_PlanarOrientation() throws Exception {
		...
		// check for gml:id
		assertXpathEvaluatesTo("er.mineraloccurrence.S0032895",
				"//er:MineralOccurrence/@gml:id", doc);
		...
		assertEquals("gsml:CGI_PlanarOrientation", gsml_planarOrientation
				.getNodeName());

		// azimuth
		Node azimuth = gsml_planarOrientation.getFirstChild();
		...
		// convention
		Node convention = azimuth.getNextSibling();
		...
		// dip
		Node dip = convention.getNextSibling();
	...
	/**
	 * 
	 * Test elements are encoded in the order as defined in the schema
	 * GeologicUnit is tested here
	 * 
	 * @throws Exception
	 */
	...
	public void testPropertyEncodingOrder_GeoLogicUnit() throws Exception {
		...
		assertEquals(1, doc.getElementsByTagName("gml:featureMember")
				.getLength());
		...
		assertXpathEvaluatesTo("-Py",
				"//gsml:GeologicUnit[@gml:id='gu.25699']/gml:name[2]", doc);

		// occurence
		Node occurence = name2.getNextSibling();
		assertEquals("gsml:occurence", occurence.getNodeName());
		assertXpathCount(
				1,
				"//gsml:GeologicUnit[@gml:id='gu.25699']/gsml:occurence[@xlink:href='urn:cgi:feature:MappedFeature:mf1']",
				doc);

		// exposureColor
		Node exposureColor = occurence.getNextSibling();
		...
		//proportion
		Node proportion = compositionPart.getFirstChild();
		...
		//role
		Node role = proportion.getNextSibling();
